...
TL;DR: 该研究提出利用Γ-谷方晶格扭曲同质双层结构模拟高温超导体的有效模型，以ZnF₂为例展示了单轨道和双轨道方晶格哈伯德模型的实现。


<details>
  <summary>Details</summary>
Motivation: 传统高温超导体（铜基、铁基和镍酸盐）均基于方晶格结构，表现出丰富的强关联现象，但现有六角晶格莫尔异质结构无法模拟这些体系。需要开发方晶格莫尔系统来模拟高温超导体的物理模型。

Method: 结合连续哈密顿量建模、第一性原理计算和Hartree-Fock平均场理论，系统研究了Γ-谷方晶格扭曲同质双层系统，以ZnF₂为候选材料进行详细分析。

Result: 发现第一莫尔带实现了单轨道方晶格哈伯德模型（模拟铜基超导体），第二和第三莫尔带实现了px,py双轨道方晶格哈伯德模型（模拟铁基超导体）。在双轨道模型的四分之一填充区发现了稳定的反铁磁轨道-铁磁绝缘相。

Conclusion: Γ-谷方晶格莫尔系统是新一代范德华异质结构，能够实现有趣的强关联物质相，为模拟和研究高温超导体物理提供了重要平台。
...
TL;DR: 该论文分析了量子临界点附近软化光学声子能否在低温下产生线性T电阻率，发现虽然临界性增强声子软化，但清洁理论中动力学处于或接近渐近T线性散射的边际边界，且声子反馈进一步削弱了低温T线性输运的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 光学声子在高温等分配机制下自然产生线性T电阻率，但其有限能隙阻碍了该机制在低温下的持续存在。研究动机是探索电子量子临界点附近的声子软化能否克服这一障碍，为奇异金属输运提供声子基解释。

Method: 首先推导了软化声子控制低温输运的模型无关判据：除了降低重整化光学能隙外，朗道阻尼声子必须获得动力学指数z_p>d。然后分析具体机制，其中声子非线性耦合到量子临界点附近的长波长电子集体模式，并应用于Ising向列情况。采用大N场论表述，分析临界性附近的声子软化增强效应。

Result: 在清洁理论中，临界性增强的声子软化使动力学处于或接近渐近T线性散射的边际边界。考虑软化声子对电子临界扇区的反馈进一步削弱了鲁棒低温T线性输运的倾向。虽然声子软化在临界点附近增强，但不足以产生强健的低温线性T电阻率。

Conclusion: 研究结果明确了声子基解释在电子临界性附近奇异金属输运中的前景和局限性：虽然量子临界性可以增强声子软化，但清洁系统中的动力学约束和声子反馈效应限制了其产生鲁棒低温线性T电阻率的能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是理解具有随机耦合的量子海森堡模型中量子涨落对自旋玻璃有序温度的影响，以及探索SYK临界性与自旋玻璃动力学之间的交叉行为。

Method: 采用大N展开方法，其中N_f表示自旋费米子的味道数。通过自洽地包含1/N_f修正到Luttinger-Ward泛函中，控制量子涨落的强度。在弱量子涨落的大N_f极限下分析高低温相变，并研究小N_f时量子涨落对有序温度的抑制效应。

Result: 在大N_f极限下，系统呈现高温顺磁相和低温自旋玻璃有序相，相变温度与N_f无关。对于小N_f，量子涨落显著抑制有序温度。系统接近SYK相时，费米子和自旋谱密度在宽频范围内显示临界行为，自旋谱密度呈现标度不变形式χ''(ω)∼sgn(ω)。在最低能量和温度下，自旋玻璃动力学主导，产生普适的亚欧姆动力学自旋磁化率χ''(ω)∼sgn(ω)√|ω|。

Conclusion: 该研究建立了一个理解SYK临界性与自旋玻璃有序之间动力学交叉的最小框架，揭示了量子涨落在抑制自旋玻璃有序温度中的重要作用，以及系统在SYK临界区和自旋玻璃区之间的普适动力学行为。
...
TL;DR: 通过MBE、ARPES和输运测量研究CeSi2薄膜厚度对重费米子电子态的影响，发现三维厚膜中观察到色散的Kondo峰和晶体电场激发卫星峰，而二维超薄膜中晶体电场激发被抑制，Kondo峰温度降低，磁电阻最大温度从~100K降至~35K。
...
Method: 结合分子束外延(MBE)、原位角分辨光电子能谱(ARPES)和输运测量，系统研究CeSi2重费米子化合物薄膜厚度依赖的电子态变化。

Result: 三维厚膜中观察到费米能级处的色散Kondo峰和晶体电场激发卫星峰；二维超薄膜中晶体电场激发被显著抑制，基态Kondo峰在较低温度下保持强度；磁电阻最大温度从厚膜的~100K降至超薄膜的~35K。

Conclusion: 维度驱动减少了Kondo过程中的晶体电场激发，与光谱结果一致；为理解强关联4f电子系统的量子限制效应提供了直接见解，为探索二维重费米子材料中的涌现现象开辟了新机会。
...
TL;DR: 研究使用DMRG方法探索了色散光学声子对轻掺杂一维光学Hubbard-SSH模型中电子配对的影响，发现声子色散显著增强了单重态结合，但并未转化为增强的超导关联，而是形成了稳健的键关联。


<details>
  <summary>Details</summary>
Motivation: SSH模型中原子的运动调制电子跃迁，在高温超导中具有强电子配对的潜力。先前研究主要关注稀掺杂或半填充模型中的无色散（爱因斯坦）声子，而实际材料中的声子具有色散特性，因此需要研究色散声子的影响。

Method: 使用密度矩阵重整化群（DMRG）方法研究轻掺杂一维光学Hubbard-SSH模型中色散光学声子的效应，通过计算各种关联函数来分析电子配对和超导关联。

Result: 观察到声子色散显著增强了单重态结合，但增强的结合并未转化为增强的超导关联，而是在研究的参数范围内形成了稳健的键关联。声子色散对这些关联有显著影响。

Conclusion: 声子色散对电子配对和关联有重要影响，在建模真实量子材料时需要超越爱因斯坦声子极限，考虑声子的色散特性。
...
TL;DR: 该研究使用温度依赖的角分辨光电子能谱研究1T-TaS2中近公度-非公度电荷密度波相变，发现该转变涉及动量依赖的谱权重新分布而非传统金属-绝缘体转变，为室温附近的电阻异常提供了微观解释。


<details>
  <summary>Details</summary>
Motivation: 1T-TaS2中的公度和近公度电荷密度波相已被充分表征，但350K附近的非公度相变缺乏直接的动量分辨电子结构信息。研究旨在通过角分辨光电子能谱追踪这一相变，理解其微观机制。

Method: 采用温度依赖的角分辨光电子能谱技术，在跨越350K相变温度范围内，系统测量1T-TaS2的电子结构变化，重点关注布里渊区中心的准粒子谱权变化。

Result: 在布里渊区中心观察到准粒子谱权抑制，与输运异常同时发生，但没有明显的完整带隙打开。相变涉及动量依赖的谱权重新分布，表现为相干性损失导致的费米面重塑，而导带色散基本保持不变。

Conclusion: 近公度-非公度相变不符合传统金属-绝缘体转变图像，而是由相干性损失驱动的电子重构过程。研究为室温电阻异常提供了微观洞察，并可能指导过渡金属二硫化物中集体电子开关的设计原则。
...
TL;DR: 提出费米-玻色子自洽嵌入框架，结合电子自洽嵌入与声子相干态平均场处理，用于处理电子-声子相互作用系统，在局域化主导区域表现优异


<details>
  <summary>Details</summary>
Motivation: 开发高效处理大尺度电子-声子相互作用系统的方法，传统方法如密度矩阵重整化群计算成本高，难以扩展到大型系统

Method: 费米-玻色子自洽嵌入框架：电子部分采用自洽嵌入方法处理关联电子，声子部分采用自洽相干态平均场处理，将电子-声子相互作用问题建模为关联电子在自洽势场中运动

Result: 在一维Hubbard-Holstein模型上验证了方法收敛性（可达350个格点），与DMRG在8格点系统对比显示数量级计算速度优势，在Mott绝缘相和强耦合小极化子区域表现最佳

Conclusion: 方法在局域化主导区域（Mott绝缘相、强耦合小极化子区）有效，但在弱耦合离域区域和Peierls相变点存在局限，主要受限于声子平均场处理无法捕捉量子涨落和长程关联
...
TL;DR: 该研究采用DFT+DMFT方法研究单层Fe₃GeTe₂在顺磁相的电子和磁性质，发现该化合物远离局域磁矩极限，具有非线性温度依赖的磁化率，并揭示了不同对称性铁原子间的RKKY交换相互作用对稳定铁磁序的关键作用。


<details>
  <summary>Details</summary>
Motivation: 研究单层Fe₃GeTe₂的电子和磁性质，特别是其部分巡游磁性行为的微观机制。传统DFT方法可能无法准确描述这种强关联材料，需要采用更精确的DFT+DMFT方法来处理电子关联效应。

Method: 采用DFT+DMFT（密度泛函理论+动力学平均场理论）方法研究单层Fe₃GeTe₂在顺磁相的电子和磁性质。通过分析不同对称性铁原子的局域磁矩、磁化率的温度依赖性，以及RKKY型交换相互作用，评估了中等库仑相互作用（U=3-4 eV）下的磁性行为。

Result: 1. Fe₃GeTe₂远离局域磁矩极限，在宽温度范围内表现出非线性的部分逆局域和均匀磁化率温度依赖性；2. 位于Ge平面上方和下方的铁原子携带显著局域磁矩（μ≳4.5 μB），而位于Ge平面内的铁原子无明显磁矩；3. 这两类不同对称性原子间的RKKY型交换相互作用对稳定长程铁磁序至关重要；4. 计算得到的自旋波刚度和居里温度与实验数据吻合良好。

Conclusion: 电子关联效应的动力学处理对于准确描述Fe₃GeTe₂的部分巡游磁性行为至关重要。该材料表现出复杂的磁性特征，其中不同对称性铁原子间的RKKY交换相互作用在稳定铁磁序中起决定性作用，DFT+DMFT方法能有效捕捉这些强关联效应。
...
TL;DR: 该论文扩展了分形子量子场论中的叶状-奇异对偶性，将其与子系统对称性的't Hooft反常流入机制相结合，在2+1维的BF理论和φ-理论中建立了叶状描述与奇异描述之间的对应关系。


<details>
  <summary>Details</summary>
Motivation: 分形子相是具有子系统全局对称性的新型物质相，其低能有效描述有两种形式：奇异张量规范场理论和叶状场论。叶状-奇异对偶性在某些分形子QFT中成立，但需要将其与子系统对称性的反常流入机制结合，以扩展该对偶性的适用范围。

Method: 1. 研究2+1维奇异和叶状BF理论，分析其ℤ_N×ℤ_N子系统对称性的混合't Hooft反常，通过在一维更高的子系统对称性保护拓扑相中捕获该反常。2. 研究2+1维奇异φ-理论（分形子无能隙标量场理论），分析其U(1)×U(1)子系统对称性的't Hooft反常，通过3+1维SSPT相捕获反常。将叶状-奇异对偶性扩展到这些理论中，建立场对应关系并构造叶状描述。

Result: 1. 建立了分形子BF理论中叶状-奇异对偶性到SSPT相的扩展，确立了SSPT相中的场对应关系，并构造了SSPT相的叶状描述。2. 在奇异φ-理论中建立了叶状-奇异对偶性，确立了场对应关系并构造了与奇异φ-理论等价的叶状φ-理论，这是叶状-奇异对偶性在无能隙理论中的首个例子。

Conclusion: 通过将叶状-奇异对偶性与子系统对称性的反常流入机制相结合，成功扩展了该对偶性在分形子量子场论中的应用范围，不仅在有能隙的BF理论中建立了对应关系，还在无能隙的φ-理论中实现了叶状描述与奇异描述之间的等价性，为分形子相的研究提供了新的理论框架。
...
TL;DR: 该论文提出了一种基于单粒子格林函数的框架，用于表征强关联系统中的相互作用拓扑相，通过有效单粒子描述来诊断关联拓扑材料。


<details>
  <summary>Details</summary>
Motivation: 理解拓扑在强关联系统中的存在是一个核心挑战，因为大多数拓扑诊断依赖于非相互作用的能带结构。需要开发与相互作用系统兼容的拓扑表征方法。

Method: 使用具有Hatsugai-Kohmoto相互作用的Su-Schrieffer-Heeger模型作为可解析处理的例子，从格林函数构建单粒子约化密度矩阵，并利用它定义有效绕数（winding number）和量子体积（quantum volume）来测量态几何。

Result: 这些量允许直接从单粒子可观测量中区分三种绝缘相，包括关联Mott态。结果表明相互作用拓扑可以通过单粒子激发的谱权重分布来解释。

Conclusion: 该方法为诊断关联系统中的拓扑相提供了直观且计算可及的途径，与现代多体模拟技术兼容，并为识别相互作用拓扑材料开辟了道路。
...
TL;DR: 在方晶格-卡格米晶格上研究交替磁相的出现，发现子晶格干涉效应使不同子晶格上的磁有序倾向解耦，产生两种不同类型的交替磁相。


<details>
  <summary>Details</summary>
Motivation: 研究交替磁相在特定晶格结构中的出现机制，探索子晶格选择性磁不稳定性如何调控交替磁序的性质。

Method: 使用Kotliar-Ruckenstein slave boson形式理论，分析方晶格-卡格米晶格上的电子相互作用，通过子晶格权重分解研究费米能级本征态的正交子晶格干涉效应。

Result: 识别出两种交替磁相：d_{xy}型和d_{x^2-y^2}型，它们源于不同的子晶格极化模式；这些交替磁相的稳定性随相互作用强度变化。

Conclusion: 子晶格选择性磁不稳定性为调控交替磁序的性质提供了灵活途径，展示了在复杂晶格结构中设计交替磁相的可能性。
...
TL;DR: UTe2中发现新的交错电荷密度波（CDW），其对外磁场表现出强烈各向异性响应，与轨道驱动机制相关，并与Kondo效应存在紧密关联。


<details>
  <summary>Details</summary>
Motivation: UTe2是一种独特的强关联量子材料，其中多种电子序共存。理解其正常态中的竞争或交织序对于阐明超导起源至关重要，但此前对其正常态电子景观的认识仍不完整。

Method: 使用扫描隧道显微镜（STM）结合矢量磁场技术，在高品质UTe2晶体中探测电子结构，通过磁场方向调控研究CDW和各向异性响应，并结合理论建模分析轨道选择性杂化机制。

Result: 发现了此前未报道的交错CDW，其对外磁场表现出强烈各向异性：沿铀链方向（a轴）仅需1.7T磁场即可完全抑制CDW，而沿其他方向则保持稳定。同一磁场条件同时改变了杂化能隙并抑制了5f Kondo共振，表明CDW与Kondo效应之间存在紧密关联。

Conclusion: 研究建立了调控UTe2中交织序的有效手段，提供了轨道选择性Kondo杂化的证据，揭示了CDW与Kondo效应之间的内在关联，为理解该材料的强关联正常态提供了新见解。
...
TL;DR: CaMn₂Sb₂在压力下经历从三角晶系到单斜晶系的结构相变，伴随体积塌缩和电子局域化，产生新的非公度磁序，揭示了结构畸变、电荷重分布和磁序耦合的关联。
...
Method: 采用单晶X射线衍射分析压力下的结构演化，通过残余电子密度分析探测电荷局域化，进行键合分析研究轨道重构，利用中子散射确定磁结构变化。

Result: 在约5.4 GPa处观察到从三角晶系P-3m1到单斜晶系P2₁/m的一级相变，伴随约7%的体积塌缩。中间压力下发现沿Mn-Sb链的电荷局域化，表明结构相变前的电子不稳定性。键合分析显示压力下Mn-Sb轨道各向异性重构，导致扭曲四方锥几何结构。中子散射确认相变并识别出压力诱导的非公度磁序，不同于环境下的反铁磁态。在单斜相中，锯齿形Mn链沿ac平面表现出反铁磁耦合，由增强的轨道重叠实现。

Conclusion: CaMn₂Sb₂成为研究压力下层状锰基磷族化合物中结构畸变、电荷重分布和磁序耦合的模型系统。研究揭示了电子离域转变附近结构、电子和磁序之间的复杂相互作用，为理解强关联材料中的相变机制提供了重要见解。
...
TL;DR: 研究使用DMRG方法分析掺杂Kitaev-Heisenberg模型在两腿梯子上的行为，发现配对倾向与空穴动能相关，构建了掺杂依赖的相图，揭示了不同区域的超导、电荷密度波和自旋密度波关联。


<details>
  <summary>Details</summary>
Motivation: 研究掺杂Kitaev自旋液体在二维受限几何结构（两腿梯子）中的物理行为，特别是探索掺杂对Kitaev-Heisenberg模型中各种量子相的影响，以及空穴动能如何调控配对倾向和量子关联。

Method: 使用密度矩阵重整化群（DMRG）方法数值研究掺杂Kitaev-Heisenberg（t-J-K）模型在两腿梯子几何结构上的行为。分析不同掺杂水平和跳跃强度下的物理性质，包括单空穴和双空穴掺杂情况。

Result: 发现配对倾向仅出现在t/K≲0.65范围内，与之前三腿梯子结果一致，表明掺杂Kitaev自旋液体的物理强烈依赖于空穴动能。构建了掺杂依赖的相图：横梯单态区域在掺杂后发展出主导的超导关联；弱掺杂时电荷密度波关联在向条纹相转变附近占主导；AFK、铁磁Kitaev极限和条纹相中表现出自旋密度波行为。

Conclusion: 掺杂Kitaev-Heisenberg模型在两腿梯子中展现出丰富的量子相行为，空穴动能是调控配对倾向的关键参数。不同相互作用区域在掺杂后表现出不同的主导关联，为理解掺杂Kitaev自旋液体的物理机制提供了重要见解。
...
TL;DR: 该论文研究描述常曲率曲面的非线性偏微分方程组，基于联络1-形式的平坦条件，对Camassa-Holm型方程组进行分类，并应用于构造新的系统实例。


<details>
  <summary>Details</summary>
Motivation: 研究描述伪球面或球面曲面的非线性偏微分方程组，特别是Camassa-Holm型方程组，旨在建立系统分类理论并发现新的可积系统实例。

Method: 利用联络1-形式的平坦条件，对具有特定形式的Camassa-Holm型方程组进行分类分析，包括二阶和三阶系统，并应用结果构造具体实例。

Result: 建立了Camassa-Holm型方程组的分类结果，发现了新的系统实例如Song-Qu-Qiao系统、具有三次非线性的双分量Camassa-Holm系统等，并构造了非局部对称性和非平凡解。

Conclusion: 通过联络平坦条件成功分类了描述常曲率曲面的Camassa-Holm型方程组，为可积系统的研究提供了新的理论框架和具体实例。
...
<details>
  <summary>Details</summary>
Motivation: 奇异弹性材料在特定载荷下表现出比其固有对称性更高的机械响应对称性，这类材料在概念和功能上位于两个不同对称类之间，具有重要的实际应用价值。它们能够实现定制化超材料设计，以调和原本不相容的机械要求，例如在固有各向异性介质中实现杨氏模量的方向各向同性。

Method: 在三维线性弹性框架内进行系统分类，通过详尽分类方法枚举所有可能的奇异结构，重点关注对称性高于正交各向异性的情况。

Result: 分类结果枚举出18种奇异结构，这些结构对应于高于正交各向异性的对称类，并对代表性奇异弹性行为进行了详细分析。

Conclusion: 该研究完成了对三维线性弹性中奇异结构的全面分类，为设计具有特殊对称性响应的超材料提供了理论基础，有助于实现原本不相容的机械性能要求。
...
<details>
  <summary>Details</summary>
Motivation: 研究受限势场中相互作用的二维量子玻色气体在高温高密度极限下的统计力学行为，建立从量子多体系统到连续场论的数学联系

Method: 通过重整化处理发散的质量和能量抵消项，证明相对配分函数的收敛性以及重整化约化密度矩阵在L¹∩L∞空间中的收敛性

Result: 成功证明了受限势场中二维量子玻色气体在高温高密度极限下收敛于复欧几里得场论，克服了与均匀环面情况不同的数学挑战

Conclusion: 该研究建立了受限势场中二维量子玻色气体与场论之间的严格数学联系，为理解量子多体系统在连续极限下的行为提供了重要理论框架
...
### [19] [Combining Symmetries and Helmholtz's Conditions to Construct Lagrangians](https://arxiv.org/abs/2603.07760)
*Merced Montesinos,Diego Gonzalez,Jorge Meza*

Main category: math-ph

TL;DR: 该论文基于诺特定理推导出新关系，将拉格朗日量的海森矩阵、对称变换与运动常数联系起来，并提出了两种将对称性要求直接融入力学逆问题的新方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于将对称性要求直接整合到力学逆问题中，从而限制可接受拉格朗日量的集合。传统方法通常先构造拉格朗日量再检查对称性，而本文旨在从源头构建具有特定对称性的拉格朗日量。

Method: 方法包括：1) 从诺特定理推导出新关系，揭示拉格朗日量海森矩阵、配置变量和时间的无穷小对称变换以及运动常数之间的兼容性；2) 将这些关系与赫尔姆霍兹条件结合，开发两种新方法将对称性要求直接融入逆问题；3) 通过一维和二维示例说明理论。

Result: 成功推导出连接拉格朗日量海森矩阵、对称变换和运动常数的新关系，并开发了两种将对称性要求直接整合到力学逆问题中的新方法。这些方法能够从源头构建具有特定对称性的拉格朗日量，并通过示例验证了理论的有效性。

Conclusion: 该研究提供了将对称性要求直接融入力学逆问题的新框架，通过诺特定理推导的关系与赫尔姆霍兹条件相结合，能够系统地构建具有预设对称性的拉格朗日量，为对称性约束下的拉格朗日量构造提供了新工具。
...
### [20] [Exceptional Optical Phonon Coherence in Enriched Cubic Boron Arsenide via Suppression of Three-Phonon Scattering](https://arxiv.org/abs/2603.11256)
...
TL;DR: 立方砷化硼(BAs)中光学声子具有极高的相干性，质量因子超过3.7×10³，主要受同位素散射而非缺陷散射限制


<details>
  <summary>Details</summary>
Motivation: 立方砷化硼因其优异的双极性迁移率和热导率成为下一代电子器件的有前景材料，但高阶非谐散射过程的理论和实验分析困难，声子寿命和热导率的极限仍不明确

Method: 使用高分辨率拉曼光谱和傅里叶变换红外光谱，通过温度依赖的阻尼率分析区分三种退相干机制，研究>98%富集的¹¹BAs晶体

Result: 在宽温度范围内，带隙几乎消除了三声子散射，导致记录性的同位素纯度限制的声子相干性，质量因子在100K以下超过3.7×10³；缺陷散射对光学声子线宽的贡献与同位素散射相比可忽略

Conclusion: 研究为BAs中光学声子的本征和非本征散射机制提供了关键见解，激励进一步量化非谐效应并实现优越的声子输运
...
### [21] [Switchable circular dichroism and ionic migration dominated charge transport in a chiral spin crossover polymer](https://arxiv.org/abs/2603.11261)
...
TL;DR: 手性自旋交叉材料中实现了热可切换的手性光学活性，在低自旋态具有显著圆二色性，高自旋态则被淬灭，同时表现出离子迁移主导的输运特性


<details>
  <summary>Details</summary>
Motivation: 探索手性自旋交叉材料中自旋态转变与手性光学活性之间的关联，以及此类材料中电荷输运的物理机制

Method: 使用磁测量确定相变特性，通过Fe L3,2边X射线吸收谱分析电子结构变化，结合电学测量研究I(V)和C(V)特性

Result: 观察到低自旋态具有显著圆二色性而高自旋态被淬灭，发现室温附近具有迟滞的协同相变，X射线吸收证实手性淬灭与Fe 3d电子结构重组相关，电学测量显示离子迁移主导输运过程

Conclusion: 手性自旋交叉材料实现了热可切换的手性光学活性，其手性淬灭与自旋态转变相关的电子结构重组直接相关，且输运过程主要由离子迁移而非电子或空穴主导
...
### [22] [Intrinsic Even-Odd Thickness-Driven Anomalous Hall in Epitaxial MnBi2Te4 Thin Films](https://arxiv.org/abs/2603.11305)
...
TL;DR: 通过分子束外延精确控制MnBi2Te4薄膜的磁性，揭示了异常霍尔效应的奇偶层数依赖性，为实现零场量子反常霍尔效应迈出重要一步。


<details>
  <summary>Details</summary>
Motivation: MnBi2Te4是一种拓扑材料，其磁性控制对于实现零场量子反常霍尔效应至关重要。然而，薄膜生长中的缺陷和厚度控制问题阻碍了对其本征磁性的研究。

Method: 采用分子束外延技术精确合成MnBi2Te4薄膜，优化Mn-Bi-Te比例和生长温度以最小化缺陷。使用X射线衍射和反射率定量表征薄膜质量和厚度，结合磁输运测量分析异常霍尔效应。

Result: 发现异常霍尔效应具有显著的奇偶层数依赖性：奇数层薄膜在尼尔温度(~25K)以下表现出大的磁滞回线，符合非补偿反铁磁性；偶数层薄膜响应极小，符合反铁磁性。异常霍尔效应的符号可用于区分本征磁性和缺陷相关磁性。

Conclusion: 该工作确定了诱导纯净非补偿铁磁性的关键因素，揭示了MnBi2Te4中本征异常霍尔效应的特性，为实现拓扑材料中的零场量子反常霍尔效应奠定了基础。
...
### [23] [Meta-generalized gradient approximation made in the Hartree gauge](https://arxiv.org/abs/2603.11517)
...
<details>
  <summary>Details</summary>
Motivation: 密度泛函理论中交换关联能密度的空间定义不唯一，导致其精确约束只能在特定规范下表述，很少用于密度泛函近似的构建。需要一种能在特定规范下构建泛函的方法，为机器学习提供更多参考数据，并为需要规范对齐的更精确非局域密度泛函建立基础

Method: 在Hartree规范下构建元广义梯度近似交换能泛函，利用氢原子的交换能密度在核心区域和渐近区域进行规范对齐，在交换能密度层面构建密度泛函近似

Result: 该方法扩展了机器学习的参考数据集，为需要规范对齐的更精确非局域密度泛函建立了基础，实现了在特定规范下构建密度泛函近似的新途径

Conclusion: 通过在交换能密度层面构建密度泛函近似，特别是在Hartree规范下利用氢原子参考系统进行规范对齐，为开发更精确的密度泛函近似提供了新框架，有望推动非局域密度泛函的发展
...
### [24] [Why ice is so slippery](https://arxiv.org/abs/2603.11539)
*Sigbjørn Løland Bore,B. N. J. Persson,Henrik Andersen Sveinsson*

Main category: cond-mat.mtrl-sci

TL;DR: 冰面摩擦的纳米尺度模拟结合摩擦加热模型，解释了冰面滑溜性的起源，验证了Bowden和Hughes的经典理论但无需融化机制。


<details>
  <summary>Details</summary>
Motivation: 解决冰面滑溜性起源这一长期存在的科学谜题，理解冰面摩擦的物理机制。

Method: 采用第一性原理进行冰-玻璃（非晶二氧化硅）纳米尺度摩擦模拟，结合摩擦加热模型将结果扩展到宏观尺度。

Result: 纳米模拟单独无法正确捕捉冰摩擦的速度依赖性，会高估摩擦系数；考虑摩擦加热后，接触温度在0.1 m/s以上速度时会显著升高接近熔点，与实验数据高度吻合。

Conclusion: 冰的最终滑溜性取决于摩擦加热，验证了Bowden和Hughes 1939年的理论，但无需融化过程；初始润滑膜形成可能无需加热，但滑溜性主要来自摩擦加热效应。
...
### [25] [Imaging antiferromagnetic domains in LiCoPO$_4$ via the optical magnetoelectric effect](https://arxiv.org/abs/2603.11541)
*B. Tóth,V. Kocsis,Y. Tokunaga,Y. Taguchi,Y. Tokura,S. Bordács*

Main category: cond-mat.mtrl-sci

TL;DR: 该论文展示了利用光学磁电效应区分反铁磁材料LiCoPO₄中两种反相畴的方法，通过可见光和红外光谱观察到自发非互易吸收（定向二向色性），并成功实现了反铁磁畴的光学成像。


<details>
  <summary>Details</summary>
Motivation: 反铁磁材料作为新型数据存储器件的有前景构建模块，但其畴结构的检测和操控一直具有挑战性。需要开发新的方法来探测和区分反铁磁畴。

Method: 使用可见光和红外光谱技术，在LiCoPO₄晶体场激发处观察自发非互易吸收（定向二向色性），该效应源于Co²⁺离子在扭曲氧八面体配位环境中的光学磁电响应。利用1550nm附近通信波段的显著吸收对比，采用简单的透射光学显微镜进行畴成像。

Result: 成功区分了LiCoPO₄中两种反相畴，在1550nm附近观察到明显的吸收差异，实现了反铁磁畴的光学成像。证明光学磁电效应可用于探测非中心对称过渡金属化合物中的反铁磁序参数。

Conclusion: 光学磁电效应为探测非中心对称过渡金属化合物中的反铁磁序参数提供了有前景的途径，特别是在通信波长附近的光学对比为反铁磁畴成像和操控开辟了新方法。
...
### [26] [High-pressure phase stability and superconductivity in La-Zr-H hydrides](https://arxiv.org/abs/2603.11590)
...
TL;DR: 通过进化结构搜索和第一性原理计算，系统研究了La-Zr-H三元体系在150-300 GPa压力下的超导性能，发现了多个稳定和亚稳定的高Tc超导相，最高Tc达209K，并建立了机器学习模型预测高Tc氢化物。


<details>
  <summary>Details</summary>
Motivation: 富氢三元氢化物在兆巴压力下具有高Tc超导潜力，但其化学空间广阔且未充分探索。需要系统研究La-Zr-H三元体系以发现新的高温超导相，并建立结构-性能关系指导后续研究。

Method: 结合进化结构搜索和第一性原理计算，在150-300 GPa压力范围内系统研究La-Zr-H三元体系。采用零点能校正的凸包分析确定热力学稳定性，通过Eliashberg方程求解预测超导转变温度，并训练随机森林机器学习模型分析结构-性能关系。

Result: 发现了多个稳定和亚稳定的超导相：R3m-Zr2H17在300 GPa下Tc=209 K，P6/mmm-LaZr2H24在200 GPa下Tc=202 K，以及亚稳定的P6m2-LaZrH18在300 GPa下Tc=206 K。所有高Tc相均具有高对称性结构、致密氢笼、有利的电子-氢比以及费米能级处大的氢衍生态密度。机器学习模型成功再现了这些结构-性能趋势。

Conclusion: La-Zr-H三元体系在高压下存在多个高Tc超导相，高对称性结构和致密氢笼是获得高Tc的关键特征。建立的机器学习模型能够有效预测潜在的高Tc氢化物，为后续计算和高压实验提供了指导。
...
### [27] [High-fidelity level-set modeling of polycrystalline grain growth](https://arxiv.org/abs/2603.11608)
*Tianchi Li,Marc Bernacki*

Main category: cond-mat.mtrl-sci

TL;DR: 提出了一种用于强异质性多晶材料晶粒生长的高保真水平集框架，特别针对具有高度异质、取向差依赖晶界能量的情况。
...
Method: 开发了一种高保真水平集框架，这是作者先前开发的单三叉点基准水平集公式的多晶扩展。该方法基于Mullins平均曲率流理论，能够处理高度异质、取向差依赖的晶界能量。

Result: 与三种现有水平集模型深入比较表明，该方法在晶粒统计、取向差分布函数和三叉点二面角演化方面表现出最高的能量一致性。在整个异质性谱范围内保持了准确性和鲁棒性。
...
### [28] [Rainbow Scattering from Graphene](https://arxiv.org/abs/2603.11672)
*Carolin Frank,Kevin Vomschee,Radek Holeňák,Yossarian Liebsch,Marika Schleberger,Daniel Primetzhofer*

Main category: cond-mat.mtrl-sci

TL;DR: 实验观测到40 keV Xe⁺离子穿过单层石墨烯时产生原子彩虹散射现象，包含六边形内彩虹和圆形外彩虹两种模式


<details>
  <summary>Details</summary>
Motivation: 研究高能离子穿过二维材料（特别是石墨烯）时的散射行为，探索原子尺度上的彩虹散射现象及其物理机制

Method: 使用飞行时间中能离子散射技术实验观测40 keV Xe⁺离子穿过自支撑单层石墨烯的散射，结合分子动力学和二元碰撞近似模拟进行理论分析

Result: 观察到石墨烯的彩虹散射图案由两部分组成：小的六边形内彩虹（源于离子与多个碳原子的特征轨迹相互作用）和较大的圆形外彩虹（源于离子与单个碳原子的近距离二元碰撞）

Conclusion: 石墨烯的原子彩虹散射具有独特的双重结构，反映了二维材料中离子-原子相互作用的复杂动力学过程，为理解低维系统中的离子散射提供了新见解
...
### [29] [Shear-Coupled Grain Growth Statistics](https://arxiv.org/abs/2603.11690)
*Caihao Qiu,David J. Srolovitz,Gregory S. Rohrer,Jian Han,Marco Salvalaglio*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究通过连续介质建模和扩散界面模拟，详细表征了晶界剪切耦合存在下二维晶粒生长的统计特征，揭示了内部应力对晶粒生长微观结构和动力学的深刻影响。


<details>
  <summary>Details</summary>
Motivation: 晶粒生长是材料微观结构演变的基本机制，晶界迁移常伴随剪切耦合现象，这会在微观结构中产生内部应力。近期研究表明这些应力对微观结构演变和晶粒生长路径具有决定性作用，但缺乏系统统计表征。

Method: 采用结合基本微观机制的晶界迁移连续介质建模和扩散界面模拟方法，对存在晶界剪切耦合的二维晶粒生长进行详细统计特征表征。

Result: 剪切耦合导致比传统曲率驱动晶粒生长更不均匀、更少等轴的微观结构，但拓扑和几何特性与实验和原子尺度观察一致。随着晶粒生长，内部应力松弛；高应力晶粒收缩更快，低应力晶粒生长更快。

Conclusion: 内部应力作为晶粒生长的固有特征，深刻改变了晶粒生长微观结构和动力学的基本特征，这与实验和原子尺度模拟结果一致，为理解多晶材料微观结构演化提供了新视角。
...
### [30] [Melting of thin silicon films: a molecular dynamics study with two machine learning potentials](https://arxiv.org/abs/2603.11722)
...
<details>
  <summary>Details</summary>
Motivation: 研究硅烯和硅薄膜的热稳定性，比较不同机器学习势能（SNAP和GAP）在描述这些系统热分解行为方面的表现

Method: 使用分子动力学方法，采用两种机器学习势能（SNAP和GAP），研究从单层硅烯到36层硅薄膜的系统，并与Stillinger-Weber势能的结果进行比较

Result: 硅烯在500K失去结构；分解温度随薄膜厚度增加而升高，在约28层达到饱和（对应SNAP模型体熔点1380K）；8层以下薄膜呈现两相共存分解，更厚薄膜则先表面熔化后完全液化；GAP势能虽对体硅更准确，但无法描述气相，导致硅烯分解为小团簇

Conclusion: 薄膜厚度显著影响硅薄膜的热稳定性，SNAP势能能合理描述从二维到三维的转变，而GAP势能虽对体相准确但在描述低维系统时存在局限性，强调了势能选择对模拟结果的重要性
...
### [31] [First-principles insights into the optoelectronic and thermoelectric properties of X3NbY4(X= Cu, Ag, Au; Y=S, Se, Te) sulvanite compounds for energy applications](https://arxiv.org/abs/2603.11733)
*Sadeya Sabnam Emo,Md. Sharear Aman,Md. Abdur Rashid,Jaker Hossain*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究使用DFT方法系统分析了X3NbY4硫钒矿硫族化合物的结构、电子、光学和输运性质，发现这些材料具有半导体特性、良好的光学吸收能力和热电性能，适合光电子和热电应用。


<details>
  <summary>Details</summary>
Motivation: 研究X3NbY4硫钒矿硫族化合物的基本物理性质，探索其在光电子和热电设备中的应用潜力，为新型功能材料开发提供理论指导。

Method: 采用基于密度泛函理论的全势线性缀加平面波方法，计算了X3NbY4化合物的结构、电子能带、态密度、弹性性质、声子色散、光学性质和热电参数。

Result: X3NbY4化合物为间接带隙半导体，带隙值在0.50-1.80 eV范围内；Cu基材料呈脆性，Ag和Au基材料呈延性；声子色散证实动态稳定性；高吸收系数达10^5 cm^-1；热电参数显示良好的热电性能。

Conclusion: X3NbY4硫钒矿硫族化合物具有优异的半导体特性、光学吸收能力和热电性能，是光电子和热电设备应用的潜在候选材料。
...
### [32] [Tunable decoupling of coexisting magnetic orders in Co$_{1/3}$TaS$_2$](https://arxiv.org/abs/2603.12072)
*Yining Hu,Zili Feng,Takashi Kurumaji,Linda Ye,Chunyu Mark Guo,Philip J. W. Moll*

Main category: cond-mat.mtrl-sci

TL;DR: 该论文报道了在反铁磁体Co₁/₃TaS₂中发现的全磁性多铁性行为，其中拓扑标量自旋手性和向列性共存于同一自旋晶格，磁场诱导二者强耦合，实现了新型多铁性行为。


<details>
  <summary>Details</summary>
Motivation: 传统多铁性材料中，晶格和磁自由度独立有序化产生耦合，本研究旨在探索全磁性的多铁性类似物，其中不同对称性的磁序共存并耦合，以实现新的物理响应和功能。

Method: 在反铁磁体Co₁/₃TaS₂中研究拓扑标量自旋手性和向列性共存体系，利用磁场诱导两种对称性不同的磁序之间的耦合，通过非互易输运作为对称性探针，在无手性几何的亚畴尺寸器件中表征全局自旋手性。

Result: 手性自旋织构产生反常霍尔效应，向列序破坏三重旋转对称性主导纵向输运。磁场诱导手性与向列序强耦合，拓扑霍尔态通过手性-向列耦合获得大的电阻异常，非互易输运可作为全局自旋手性的对称性探针。

Conclusion: Co₁/₃TaS₂展示了通过混合磁序实现高级功能的新途径，其中对称性不同的磁序耦合可合并对称性禁止的材料响应，实现了通过调节耦合本身而非直接操纵次级序的新型多铁性行为。
...
### [33] [Phase Separation in Heritage Objects Made of Plasticised PVC: the Case of Joseph Beuys Multiples](https://arxiv.org/abs/2603.11776)
...
TL;DR: 该研究采用多分析方法探讨了约瑟夫·博伊斯作品《磷-十字雪橇》中增塑PVC板的降解机制，重点关注增塑剂迁移和渗出现象，并通过DFT模拟提供分子层面的解释。


<details>
  <summary>Details</summary>
Motivation: 研究约瑟夫·博伊斯作品《磷-十字雪橇》中增塑PVC板的严重降解问题，特别是表面渗出粘性液体的现象，旨在理解其降解机制并为文物保护提供科学依据。

Method: 采用色谱、光谱、显微和机械测试相结合的多分析方法，表征渗出层和聚合物本体；结合密度泛函理论模拟研究增塑剂迁移的热力学驱动力；应用NMR光谱研究增塑剂加速迁移。

Result: DFT模拟表明增塑剂向PVC表面迁移存在热力学驱动力且倾向于自聚集；NMR光谱被证明是研究增塑剂加速迁移的有效方法；研究揭示了增塑PVC的完整降解路径。

Conclusion: 该研究为增塑PVC的渗出现象提供了分子机制解释，建立了完整的降解路径模型，NMR方法为开发非破坏性原位预防性保护工具奠定了基础，有助于早期识别有相分离风险的PVC文物。
...
### [34] [Cold source field-effect transistor with type-III band-aligned HfS$_2$/WTe$_2$ heterostructure](https://arxiv.org/abs/2603.11819)
...
<details>
  <summary>Details</summary>
Motivation: 现有冷源场效应晶体管设计面临金属-半导体界面肖特基势垒的挑战，需要开发新的冷源设计来解决这一问题

Method: 提出基于2D WTe2/HfS2异质结的冷源场效应晶体管设计，利用III型能带对齐实现高带间输运机制，采用第一性原理量子输运建模进行性能预测

Result: 预测获得约10^10的高开关电流比，在宽栅压范围内实现低于热极限的亚阈值摆幅（最小41.3 mV/dec），在低源漏偏压V_DS=0.3V下工作

Conclusion: 该工作建立了利用2D范德华异质结构设计下一代低功耗纳米电子开关的设计原则，解决了现有冷源场效应晶体管的肖特基势垒问题
...
### [35] [Origin and Propagation of Spin-orbit Torques in Pt/Co/Cu/NiFe/Capping Multilayers](https://arxiv.org/abs/2603.11843)
*Yuming Bai,Rulin Tian,Yue Zhang,Tao Wang*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究通过NiFe厚度依赖的自旋轨道转矩(SOT)效率分析，揭示了阻尼型(DL)和场型(FL)SOT在多层膜中的起源和传播特性差异。


<details>
  <summary>Details</summary>
Motivation: 自旋轨道转矩(SOT)为低功耗自旋电子器件提供了有前景的途径，但在复杂多层结构中DL和FL SOT的起源和传播机制尚不清楚。需要明确区分体相和界面贡献，并理解不同SOT分量的传播特性。

Method: 采用Ta/Pt/Co/Cu/NiFe/Cu/Capping多层结构，通过自旋转动几何配置，利用垂直磁化的Pt/Co/Cu堆栈作为自旋源，消除Oersted场贡献。引入样品面积归一化磁矩m = mNiFe/S来区分体相和界面贡献。使用平面霍尔效应和极向磁光克尔效应测量明确分离SOT分量，并比较不同覆盖层(Pt、Al、SiO2)的影响。

Result: DL-SOT遵循近似线性的1/m标度关系，表明Cu/NiFe界面的快速自旋吸收，但在Pt和Al覆盖样品中当1/m趋近零时存在有限的β_SOT，表明Cu/Pt和Cu/Al界面存在额外的自旋电流贡献。SiO2覆盖样品则显示可忽略的界面贡献。FL-SOT明显偏离1/m标度，表现出比DL-SOT更长的自旋退相干长度(约1.7nm)，表明其在NiFe中更长的传播距离。覆盖层比较研究进一步证实了界面依赖的自旋输运行为。

Conclusion: 该研究阐明了DL和FL转矩的起源和不同的传播特性，为超薄金属异质结构中界面自旋轨道功能的工程设计提供了指导。DL-SOT主要受界面自旋吸收控制，而FL-SOT表现出更长的传播距离，表明其传播机制不同。界面工程可以调控SOT效率，这对优化自旋电子器件性能至关重要。
...
### [36] [Emergence of polar monoclinic phase in heterohalogen substituted CsGeX$_3$](https://arxiv.org/abs/2603.11844)
*Sourabh Vairat,Balachandra G. Hegde,Brajesh Tiwari,Ravi Kashikar*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究通过第一性原理计算分析了CsGeX₃卤化物钙钛矿中异卤素取代对铁电性能的影响，发现2:1异卤素取代增强了结构畸变，产生室温极性单斜相，提高了极化强度，并在能带结构中观察到显著的自旋分裂和Rashba自旋纹理，使其成为Datta-Das自旋晶体管的潜在候选材料。


<details>
  <summary>Details</summary>
Motivation: 无机锗基卤化物钙钛矿中铁电性的发现为氧化物替代提供了新选择。研究旨在通过异卤素取代调控CsGeX₃材料的铁电性能和自旋相关特性，探索其在自旋电子器件中的应用潜力。

Method: 采用第一性原理计算方法，研究CsGeX₃材料在X位点的2:1异卤素取代。通过分析原子位移对应的软模本征矢沿不同方向产生的结构能量学，确认单斜相的形成。通过电子结构分析评估自旋分裂能和自旋纹理特性。

Result: 异卤素取代显著改变八面体环境，产生室温极性单斜相。极化沿[101]方向，比原始材料提高10-15%。价带自旋分裂能25-250 meV，导带9-80 meV。观察到与极化方向耦合的Rashba和持久自旋纹理。对称性相关的k·p哈密顿量参数揭示了自旋分裂强度和自旋纹理性质。

Conclusion: CsGeX₃材料通过异卤素化学调控实现了增强的铁电性能和显著的自旋分裂特性，其半导体性质和自旋极化特性使其成为Datta-Das自旋晶体管的理想候选材料，为自旋电子器件应用提供了新平台。
...
### [37] [Symmetry-Driven Floquet Engineering in Multivalley SnS](https://arxiv.org/abs/2603.11878)
...
TL;DR: 该研究利用时间分辨极紫外光电子能谱、群论分析和含时密度泛函理论，在块体多谷半导体SnS中实现了基于对称性的Floquet工程，通过驱动光偏振与晶轴相对取向的调控，实现了对Floquet-Bloch态宇称的确定性控制。


<details>
  <summary>Details</summary>
Motivation: 周期性电磁场与材料的相干相互作用为工程具有定制功能的光-物质混合Floquet态提供了强大平台。特别是操纵此类Floquet态波函数对称性的能力，已成为量子材料非平衡控制领域的新前沿。研究旨在探索对称性驱动的Floquet工程，建立对Floquet-Bloch态宇称的确定性控制。

Method: 采用时间、偏振和角度分辨的极紫外光电子能谱技术，结合群论分析和含时密度泛函理论，研究块体多谷半导体SnS中的对称性驱动Floquet工程。通过非平衡线性二向色性测量探测平衡能带和光诱导Floquet态的对称性驱动光电子选择规则。

Result: 材料固有对称性产生了显著的对称性驱动光电子选择规则。通过晶体对称性与驱动光对称性的相互作用，实现了对Floquet-Bloch态宇称的确定性控制。Floquet-Bloch态的对称性可通过驱动偏振与晶轴相对取向完全控制，实现了相对于平衡价带和导带的选择性宇称反转。对称性驱动的宇称工程还实现了偏振和谷选择性能带重整化。

Conclusion: 这些发现推进了对波函数对称性工程指导原则的理解，为使用定制电磁场选择性控制量子材料中电子态的宇称和重整化提供了途径。研究展示了通过对称性工程在非平衡条件下调控量子材料电子性质的可行性。
...
### [38] [940-nm VCSELs grown by molecular beam epitaxy on Ge(001)](https://arxiv.org/abs/2603.11902)
...
TL;DR: 首次在Ge衬底上通过MBE生长出940 nm VCSEL，采用MOVPE生长的GaAs/Ge虚拟衬底，通过原位监测实现实时生长控制，器件在室温连续波下阈值电流低于3 mA。


<details>
  <summary>Details</summary>
Motivation: 探索在Ge衬底上单片集成940 nm VCSEL的可行性，利用MBE生长技术结合原位监测，为可扩展的光电集成提供新途径。

Method: 采用固体源MBE在Ge(001)衬底上生长VCSEL结构，使用MOVPE生长的GaAs/Ge虚拟衬底，通过多光谱反射率和曲率成像进行原位实时监测。

Result: 成功制备出直径35-40 μm的器件，氧化孔径约11-16 μm，在室温连续波偏置下实现激光发射，阈值电流低于3 mA，这是首次在Ge衬底上MBE生长单片集成940 nm VCSEL的演示。

Conclusion: 证实了MBE生长VCSEL在Ge衬底上的可行性，结合原位工艺控制，为可扩展的光电集成提供了有效技术路径。
...
### [39] [Depth-resolved magnetization dynamics in Fe thin films after ultrafast laser excitation](https://arxiv.org/abs/2603.11913)
...
TL;DR: 通过时间分辨X射线共振磁反射技术，同时探测激光激发铁磁Fe薄膜的瞬态结构和磁性深度分布，发现激光激发后磁化强度在界面附近呈现强非均匀性，同时存在局部和非局域角动量转移现象。


<details>
  <summary>Details</summary>
Motivation: 研究激光激发后铁磁薄膜中瞬态结构和磁性的深度分布演化，特别是界面附近的非均匀磁化行为，以及局部和非局域角动量转移现象的共存机制。

Method: 采用时间分辨X射线共振磁反射测量技术，结合飞秒激光激发，实现纳米空间分辨和飞秒时间分辨的同步探测；通过微观三温度模型和激光诱导自旋电流模拟进行理论对比。

Result: 激光激发后前几皮秒内，Fe层磁化强度呈现强非均匀分布，特别是在埋藏界面附近；同时存在局部和非局域角动量转移现象；几皮秒后磁化弛豫回平衡态，同时薄膜厚度开始周期性振荡，最大膨胀达整个薄膜厚度的约1.3%。

Conclusion: 激光激发铁磁薄膜中同时发生局部和非局域角动量转移过程，导致界面附近磁化强非均匀性；结构响应表现为厚度周期性振荡，揭示了光致应力的重要影响。
...
### [40] [Revealing 3D orientation and strain heterogeneity in calcite generated by bio-cementation](https://arxiv.org/abs/2603.11932)
...
TL;DR: 结合显微断层扫描、3DXRD和DFXM技术，首次无损表征了生物胶结中单个方解石键的3D形态、取向织构和内部应变状态


<details>
  <summary>Details</summary>
Motivation: 生物胶结利用细菌诱导的方解石结合砂粒，提供低碳土壤稳定方法，但单个方解石键的3D形态、取向织构和内部应变状态仍缺乏充分表征

Method: 结合计算显微断层扫描、3D X射线衍射（3DXRD）和暗场X射线显微镜（DFXM），无损表征生物胶结过程中砂粒-砂粒接触处形成的方解石的晶粒形态、晶体学取向以及II型（晶间）和III型（晶内）弹性应变

Result: 断层扫描建立样品形态和胶结接触结构；3DXRD提供晶粒平均取向和应变状态；DFXM以100 nm分辨率解析生长过程中产生的亚晶粒取向差和局部应变集中

Conclusion: 生物胶结过程中的方解石沉淀产生各向异性的内部应变和独特的亚域结构，这些因素会影响键合完整性和宏观尺度上的荷载传递
...
### [41] [Irradiation-induced amplification of electric fields at oxide interfaces as revealed by correlative DPC-STEM and DFT](https://arxiv.org/abs/2603.12153)
*Elizabeth A. Peterson,Dongye Liu,Sean H. Mills,Tiffany C. Kaspar,Hyosim Kim,Yongqiang Wang,Blas P. Uberuaga,Andrew M. Minor*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究通过第一性原理计算和实验技术，揭示了辐照如何调控氧化物异质界面处的电场分布，为设计极端环境下耐腐蚀材料提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 现代设备中的异质界面在极端环境（如辐照和腐蚀同时作用）下性能退化问题突出。氧化物作为金属的保护涂层，其界面特性对材料耐腐蚀性至关重要。需要深入理解辐照诱导的非平衡缺陷如何影响界面电荷分布和电场，以设计更耐腐蚀的材料。

Method: 采用第一性原理DFT电子结构建模，结合4D-STEM DPC（差分相位衬度）和EELS（电子能量损失谱）技术，对辐照后的Fe2O3-Cr2O3薄膜异质结构进行详细研究，测量纳米尺度的电场变化。

Result: 辐照显著调制界面电场，且可通过控制氧化物界面的原子级化学结构来调控。辐照能选择性地诱导内建电场并改变其方向，这为通过电场控制缺陷空间分布提供了可能。

Conclusion: 通过工程化设计氧化物异质结构覆盖层，利用电场控制缺陷分布，为极端环境下耐腐蚀材料的设计提供了新策略，具有重要的应用前景。
...
TL;DR: 该研究通过第一性原理计算分析了三种化学结构相似但自旋弛豫时间不同的Yb(III)配合物的自旋-声子动力学，发现低温弛豫由少数高度离域的低能声子驱动的拉曼过程主导，分子结构对自旋-声子耦合的调控高度复杂且难以用简单化学术语解释。


<details>
  <summary>Details</summary>
Motivation: Yb(III)配合物具有4f化合物中最长的自旋相干时间，是分子量子技术的理想平台。虽然自旋-声子弛豫即使在低温下仍然是相干时间的限制因素，但通过化学设计控制这一过程有望将自旋量子比特原型推向当前极限之外。本研究旨在为如何化学控制自旋-声子弛豫提供见解。

Method: 对三种化学差异极小但自旋弛豫时间定量不同的Yb(III)分子进行了完整的从头算研究，分析了自旋-声子动力学。重点研究了低温弛豫机制，特别是拉曼过程及其与特定声子模式的关系。

Result: 研究发现低温弛豫由少数高度离域的低能声子驱动的拉曼过程主导。分析表明，超出第一配位壳层的分子结构修饰对自旋-声子耦合的调控具有高度非平凡性质，难以用简单的化学术语合理化。不同分子间自旋弛豫时间的差异与特定声子模式的自旋-声子耦合强度直接相关。

Conclusion: 研究结果表明，需要从试图使用简单的磁结构相关性来解释分子结构修饰对自旋-声子弛豫影响的概念转变。预测性第一性原理框架应成为未来化学设计策略的潜在驱动力，因为分子结构对自旋-声子耦合的调控机制复杂且难以通过简单化学直觉预测。
...
### [43] [Observation of Iso-Symmetric Structural and Lifshitz Transitions in Quasi-one-dimensional CrNbSe$_5$](https://arxiv.org/abs/2603.12169)
*Mingyu Xu,Peng Cheng,Shuyuan Huyan,Wenli Bi,Su-Yang Xu,Sergey L. Bud'ko,Paul C. Canfield,Weiwei Xie*

Main category: cond-mat.mtrl-sci

TL;DR: 压力诱导CrNbSe₅发生同对称结构转变，通过连续重组局部键合相互作用实现半导体-半金属态的可逆切换，为压力调控电子功能提供新范式。


<details>
  <summary>Details</summary>
Motivation: 探索准一维过渡金属硫族化合物中压力诱导的电子态调控机制，研究如何在保持晶体对称性的前提下通过键合重组实现电子基态的可逆切换。

Method: 采用高压单晶X射线衍射技术，直接解析Cr-Se和Nb-Se键距、配位多面体和连接性的演化，监测压力下键合重排过程。

Result: 压力可逆地调控CrNbSe₅在半导体和半金属态之间转变，实现低载流子和高载流子电子态的可控切换，整个过程保持晶体对称性不变。

Conclusion: CrNbSe₅成为通过可调化学键合实现电子驱动相切换的模型体系，同对称键重组为压力控制电子和自旋电子功能提供了新的设计原理。
...
TL;DR: 该论文提出了Proof-Carrying Materials (PCM)框架，通过对抗性证伪、置信区间精炼和形式化验证三个阶段，显著提升机器学习原子间势(MLIPs)在材料筛选中的可靠性，将单MLIP方法遗漏的93%稳定材料重新发现。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习原子间势(MLIPs)在高通量材料筛选中缺乏形式化可靠性保证。研究表明，单个MLIP作为稳定性过滤器会遗漏93%的DFT稳定材料，存在严重的可靠性问题，需要系统化解决方案。

Method: 提出Proof-Carrying Materials (PCM)框架，包含三个阶段：1) 在成分空间进行对抗性证伪；2) 使用95%置信区间的自举包络精炼；3) Lean 4形式化验证。该方法还训练风险模型预测未见材料的失败概率。

Result: PCM显著提升了材料发现效率：在25,000材料基准上，单MLIP的召回率仅0.07，而PCM填补了这一差距。审计CHGNet、TensorNet和MACE发现架构特异性盲点（误差相关性r≤0.13）。风险模型在未见材料上AUC-ROC达0.938±0.004，跨架构可迁移性良好。在热电材料筛选中，PCM协议多发现62个稳定材料，提升25%发现率。

Conclusion: PCM框架为MLIPs提供了形式化可靠性保证，显著改善了高通量材料筛选的准确性和可靠性。该方法揭示了不同MLIP架构的互补性盲点，并通过风险模型实现了跨架构的失败预测，为材料发现提供了系统化审计工具。
...
### [45] [A blended approach for evolving phase fields using peridynamics: Cyclic loading in quasi-brittle fracture](https://arxiv.org/abs/2603.12210)
*Hayden Bromley,Robert Lipton*

Main category: cond-mat.mtrl-sci

TL;DR: 提出了一种结合塑性变形和损伤软化的准脆性材料损伤断裂场理论，通过两点历史相关相场描述材料退化，建立了非局部本构关系，满足能量平衡和热力学定律。


<details>
  <summary>Details</summary>
Motivation: 传统准脆性材料损伤断裂模型难以同时考虑塑性变形和弹性模量损伤软化效应，需要发展能够统一描述不可逆变形与损伤演化的理论框架。

Method: 采用两点历史相关相场方法，结合牛顿第二定律描述变形演化，建立空间和时间非局部本构关系；应变采用弹性应变与不可逆应变的加性分解；应力-应变行为由强度包络线和基于损伤与塑性的卸载规律描述。

Result: 理论满足能量平衡和热力学第二定律，断裂能直接由能量释放率与裂纹面积乘积给出且与非局部长度尺度无关；模拟结果与混凝土三点弯曲试验在滞后效应和损伤方面定量定性一致，并能预测准脆性材料尺寸效应。

Conclusion: 该场理论成功统一了准脆性材料的塑性变形与损伤软化效应，通过两点历史相关相场和非局部本构关系实现了对复杂断裂模式的预测，为无网格方法模拟裂纹扩展提供了理论基础。
...
### [46] [Hidden polar phase in the quantum paraelectric SrTiO3](https://arxiv.org/abs/2603.12239)
...
TL;DR: 在SrTiO3中发现由应变稳定的新型极性态，其特征是纳米尺度的极化调制而非传统铁电性


<details>
  <summary>Details</summary>
Motivation: 量子材料中的隐藏相存在于平衡相图之外，具有奇异性质但难以识别，因为常与已知态相似。SrTiO3冷却时表现出铁电性特征却从未发展出该有序，是这一挑战的典型代表。

Method: 结合机械应变、超快激光脉冲和X射线散射技术，探测SrTiO3中的集体激发

Result: 发现SrTiO3中存在与铁电性不同的新型极性态，其特征是具有纳米波长的独特极性振动，表明应变稳定了以纳米尺度极化调制为特征的隐藏态

Conclusion: 该发现可能为量子顺电性提供替代解释，并证明在有限动量下探测集体激发对于识别量子材料中的隐藏相至关重要
...
### [47] [Emergent Anomalous Hall Effect from Surface States in the Altermagnet MnTe Thin Films](https://arxiv.org/abs/2603.12259)
...
<details>
  <summary>Details</summary>
Motivation: 针对MnTe薄膜中反常霍尔效应测量结果存在矛盾（符号相反、厚度无关电阻率）的问题，需要分离体相和表面贡献以解释这些差异，并为反铁磁薄膜的界面设计提供理论指导。

Method: 采用第一性原理计算和基于对称性的有效模型，分析不同晶面终止下的体相和表面贡献，考虑实际界面化学（Te覆盖层和InP衬底）对表面AHE的影响。

Result: 体相具有特征性的g波费米面，而表面态在体相带隙内获得类铁磁自旋极化，在实验相关费米能级主导AHE。表面磁化遵循表面自旋亚晶格，但AHE由给定终止面的体相Néel序唯一决定。Te覆盖层可反转表面AHE符号。

Conclusion: 研究建立了通过界面设计解释和调控反铁磁薄膜AHE响应的微观框架，揭示了表面态在实验相关能量下主导AHE，且界面化学可调控AHE符号，为反铁磁器件的工程应用提供指导。
...
### [48] [Pattern stability in reaction-diffusion systems depends on path entropy](https://arxiv.org/abs/2603.11192)
*Eric R. Heller,David T. Limmer*

Main category: cond-mat.stat-mech

TL;DR: 开发非平衡瞬子框架，通过最优跃迁路径及其涨落高效计算亚稳态图案间的跃迁速率，揭示路径熵在有限粒子数下改变非平衡图案稳定性的关键作用。


<details>
  <summary>Details</summary>
Motivation: 远离热力学平衡的反应扩散系统可维持多种空间图案作为长寿命动力学相。这些亚稳态的稳定性不由热力学决定，而由连接它们的跃迁路径决定。有限粒子数下，内在随机性导致竞争图案间的稀有跃迁，使连续平均场描述不足，而精确随机模拟在空间扩展系统中计算成本过高。

Method: 开发非平衡瞬子框架，通过单一最优跃迁路径及其涨落高效计算亚稳态图案间的跃迁速率。该方法避免了计算昂贵的精确随机模拟，同时克服了连续平均场描述的局限性。

Result: 路径空间中的有效熵可在有限粒子数下通过增加亚稳态图案的逃逸速率来定性改变稳定性。通过研究不同复杂度的模型，证实路径熵是非平衡图案形成的关键组织原则。

Conclusion: 非平衡瞬子框架为高效计算亚稳态图案间的跃迁速率提供了理论工具，揭示了路径熵在有限粒子数下对非平衡图案稳定性的决定性影响，确立了路径熵作为非平衡图案形成的关键组织原则。
...
### [49] [Unraveling anomalous relaxation effects in the thermodynamic limit](https://arxiv.org/abs/2603.11326)
*Emilio Pomares,Víctor Martín-Mayor,Antonio Lasanta,Gabriel Álvarez*

Main category: cond-mat.stat-mech

TL;DR: 该论文解决了反常Mpemba弛豫理论中的两个核心问题：扩展到多维空间和热力学极限下的自洽表述。通过研究外磁场下反铁磁Ising模型的弛豫动力学，揭示了在相变附近，弛豫时间谱的连续特性，并提出用序参量磁化率来表征最慢时间尺度，从而预测了多种反常弛豫现象的最优协议。


<details>
  <summary>Details</summary>
Motivation: 解决反常Mpemba弛豫理论中的两个核心开放问题：1）将理论扩展到一维以上空间维度；2）在热力学极限下建立自洽的理论框架。传统单指数弛豫假设对强度可观测量不适用，需要发展新的理论框架来理解多维系统中的反常弛豫现象。

Method: 采用外磁场下的反铁磁Ising模型作为理论框架，该模型具有丰富的相图（温度和磁场两个控制参数）。提出在相变附近，最慢时间尺度的谱投影可以用平衡热力学量——亚稳态相序参量的磁化率来有效表征。结合相图丰富性，推导出同时改变温度和磁场的最优协议，并通过蒙特卡洛模拟验证理论预测。

Result: 理论预测了多种反常弛豫现象的最优协议，包括直接和逆Mpemba效应、冷却-加热不对称性、以及预冷诱导的更快加热现象。蒙特卡洛模拟验证了这些预测，并且经过最小化后优化，可以将分析指导的协议转化为完全最优协议，使反常弛豫现象以最显著形式呈现。

Conclusion: 该研究成功将反常Mpemba弛豫理论扩展到多维系统和热力学极限，揭示了弛豫时间谱的连续特性，并提出用平衡热力学量表征最慢时间尺度的有效方法。这为理解和设计涉及温度和磁场同时变化的反常弛豫现象提供了理论框架和实用指导。
...
### [50] [Time irreversibility and entropy production in non-Hermitian Model A field theories](https://arxiv.org/abs/2603.11450)
*Matthias Carosi,Ot Garcés,Adrià Garcés,Demian Levis*

Main category: cond-mat.stat-mech

TL;DR: 该论文开发了一个系统框架来量化具有非厄米项的标量Model A场理论中的不可逆性，通过小噪声展开计算熵产生率和涨落耗散定理违反，并应用于非互易伊辛模型的粗粒化描述。


<details>
  <summary>Details</summary>
Motivation: 研究非厄米标量场理论中的时间反演对称性破缺和不可逆性，为理解非厄米系统中热力学不可逆性提供量化框架。

Method: 使用随机路径积分形式，进行受控的小噪声展开，计算熵产生率和涨落耗散定理违反；分析非厄米Ginzburg-Landau ψ^4理论的扩展，描述粗粒化尺度的非互易伊辛模型。

Result: 局部熵产生率完全由线性化朗之万方程的非厄米部分决定；在稳态附近，非厄米分量对涨落耗散定理违反产生线性修正，并对熵产生率贡献二次项；在非均匀态中，熵产生率在界面处局域化。

Conclusion: 该框架为量化非厄米标量场理论中的时间反演对称性破缺提供了通用方法，揭示了非厄米分量如何驱动不可逆性，并在界面处产生局域化效应。
...
### [51] [Skyrmion-Bimeron Transformation in Bilayer Chiral Magnets with Competing Magnetic Anisotropy](https://arxiv.org/abs/2603.11585)
*Gülşen Doğan,Ümit Akıncı*

Main category: cond-mat.stat-mech

TL;DR: 通过蒙特卡洛模拟研究双层手性磁体中拓扑自旋纹理的涌现，构建了(K/J, h/J)参数空间的标量手性图，揭示了多种磁构型及从斯格明子到双子型构型的连续转变。


<details>
  <summary>Details</summary>
Motivation: 研究双层手性磁体中拓扑自旋纹理的涌现机制，探索层间交换耦合对拓扑纹理稳定性的影响，为纳米尺度自旋电子学系统提供稳定的双子型自旋纹理平台。

Method: 采用蒙特卡洛模拟经典自旋模型，包含交换相互作用、Dzyaloshinskii-Moriya相互作用、磁各向异性和外磁场。在(K/J, h/J)参数空间构建标量手性图来表征系统拓扑性质。

Result: 发现多种磁构型：迷宫结构、斯格明子晶格、铁磁态、子-反子晶体相。从易轴到易面各向异性的转变驱动了从斯格明子纹理到双子型构型的连续转变。双层几何通过层间交换耦合关联两层拓扑核，增加缺陷塌缩的能量成本。

Conclusion: 为双层手性磁体提供了系统的拓扑纹理图，强调耦合磁层作为稳定纳米尺度自旋电子学系统中双子型自旋纹理的有前景平台。
...
### [52] [What is a minimum work transition in stochastic thermodynamics?](https://arxiv.org/abs/2603.11995)
*Paolo Muratore-Ginanneschi,Julia Sanders*

Main category: cond-mat.stat-mech

TL;DR: 重新评估经典随机有限时间热力学中的最小功转变概念，证明最优控制问题的良好表述必须考虑控制协议的速度限制，这具有重要定性影响。


<details>
  <summary>Details</summary>
Motivation: 重新审视经典随机有限时间热力学中最小功转变的概念，揭示在扩散过程建模的系统动力学中，最优控制问题的良好表述需要包含控制协议的速度限制，这对区分最优快速工程平衡与最小功转变至关重要。

Method: 采用扩散过程建模系统动力学，通过最优控制理论分析有限时间转变中的平均功最小化问题，特别关注控制协议的速度限制条件，并以高斯移动陷阱中的悬浮粒子作为模型问题进行说明。

Result: 证明考虑速度限制是构建良好最优控制问题的必要条件；能够区分最优快速工程平衡与最小功转变；在速度限制移除的极限情况下，只有广义薛定谔桥指定的转变具有一致的物理解释。

Conclusion: 在经典随机有限时间热力学中，最小功转变概念需要重新评估，控制协议的速度限制是构建物理一致的最优控制问题的关键要素，对区分不同类型的转变过程具有重要意义。
...
### [53] [Weak integrability breaking perturbations in classical integrable models on the lattice](https://arxiv.org/abs/2603.11712)
*Sara Vanovac,Catherine McCarthy,Federica Maria Surace,Olexei I. Motrunich*

Main category: cond-mat.stat-mech

TL;DR: 该论文提出了一个系统构建经典可积晶格模型中弱可积破坏微扰的框架，并应用于Ishimori模型、Toda链和简谐振子链，特别证明了alpha-FPUT模型的立方非线性是真正的弱可积破坏微扰。


<details>
  <summary>Details</summary>
Motivation: 量子系统中的弱可积破坏微扰已被广泛研究，但计算长时动力学具有挑战性。经典可积晶格模型为研究弱可积破坏微扰下的输运和长时动力学提供了互补平台，需要系统构建方法来量化其对反常输运的贡献。

Method: 开发了一个通用框架来系统构建经典可积模型的弱可积破坏微扰，特别应用于Ishimori模型、Toda链和简谐振子链。利用Toda晶格的守恒量，显式构造了简谐振子链守恒量层次的修正，得到了alpha-FPUT链的无限塔状准守恒量。进一步识别了对应的绝热规范势作为非平凡的三局域生成元。

Result: 成功构建了多个弱可积破坏微扰家族，证明了alpha-FPUT模型的立方非线性是真正的弱可积破坏微扰。发现任何立方、平移不变、动量守恒的简谐振子链微扰都允许这样的生成元，因此是弱可积破坏微扰。通过输运和绝热规范势方差研究，为弱可积破坏和诊断近可积哈密顿晶格系统中的反常热化和输运提供了统一框架。

Conclusion: 该工作为经典可积模型中的弱可积破坏微扰提供了系统构建方法，建立了统一的经典框架来研究近可积系统中的反常热化和输运现象，特别阐明了alpha-FPUT模型作为弱可积破坏微扰的理论基础。
...
### [54] [Quantum Many-Body Mpemba Effect through Resonances](https://arxiv.org/abs/2603.11788)
*Shion Yamashika,Ryusuke Hamazaki*

Main category: cond-mat.stat-mech

TL;DR: 该论文提出了一个理解封闭量子多体混沌系统中量子Mpemba效应（QME）的通用框架，通过Ruelle-Pollicott共振重新表述局部子系统的平衡过程，揭示了QME的微观机制。


<details>
  <summary>Details</summary>
Motivation: 量子Mpemba效应（QME）作为经典Mpemba效应的量子类比，在可控量子平台中已被探索，但其微观起源仍不清楚。需要建立一个通用框架来理解封闭量子多体混沌系统中的QME机制。

Method: 通过Ruelle-Pollicott（RP）共振重新表述局部子系统的平衡过程，分析初始态与主导RP共振模式的重叠抑制如何加速子系统平衡。使用典型的kicked Ising链和受数论启发的实验相关初始态来验证预测。

Result: 研究表明，抑制初始态与主导RP共振模式的重叠可以加速子系统平衡，从而产生QME。进一步发现，通过初始态的完全平移对称性破缺可以产生新型强QME。这些预测在kicked Ising链模型中得到验证。

Conclusion: 该研究为封闭多体系统中的QME提供了一个统一框架，将其与开放系统类比联系起来，并为最先进量子平台提供了实验可观测的特征。这些发现深化了对量子弛豫动力学的理解。
...
### [55] [A Universality Emerging in a Universality: Derivation of the Ericson Transition in Stochastic Quantum Scattering and Experimental Validation](https://arxiv.org/abs/2603.12068)
*Simon Köhnes,Jiongning Che,Barbara Dietz,Thomas Guhr*

Main category: cond-mat.stat-mech

TL;DR: 论文推导了量子混沌多体系统中从孤立共振到Ericson散射机制的过渡，证明了散射矩阵元素的高斯分布，并给出了分布的显式矩公式。


<details>
  <summary>Details</summary>
Motivation: 量子混沌多体系统中，散射共振在低能时孤立，高能时重叠形成Ericson机制，散射截面呈现随机函数行为。60多年来，这种在系统随机性基础上出现的额外普适行为缺乏简洁的解析处理。

Method: 采用普适的Heidelberg方法，通过适当的渐近展开推导Ericson机制的过渡，证明散射矩阵元素的普适高斯分布。

Result: 获得了分布矩的显式公式，并与微波实验和数值模拟进行了比较验证。

Conclusion: 成功推导了量子混沌多体系统中Ericson机制的过渡，证明了散射矩阵元素的普适高斯分布，为这一长期未解决的物理问题提供了简洁的解析处理。
...
### [56] [Spatiotemporal Characterization of Active Brownian Dynamics in Channels](https://arxiv.org/abs/2603.12080)
*Yanis Baouche,Mathis Guéneau,Christina Kurzthaler*

Main category: cond-mat.stat-mech

TL;DR: 该论文研究了活性布朗粒子在边界处的积累现象，通过Siegmund对偶性建立了吸收边界和硬壁边界问题之间的映射关系，分析了不同活性区域下的首次通过特性和空间分布。


<details>
  <summary>Details</summary>
Motivation: 边界积累是活性系统中广泛观察到的现象，对微生物生态学和工程应用具有重要意义。需要从物理角度理解这一现象，特别是活性布朗粒子在受限条件下的首次通过特性和空间分布。

Method: 采用活性布朗粒子模型，分析吸收边界和硬壁边界条件下的传播子。利用Siegmund对偶性建立两种边界条件问题之间的直接映射关系。研究低活性区和高活性区（即持续运动相对于扩散的相对重要性）的行为。

Result: 活性运动结合有利的初始方向通常能降低相对于被动扩散的平均首次通过时间。硬壁间的全时变传播子通过Siegmund对偶性趋近于由分裂概率导数给出的壁积累稳态。

Conclusion: Siegmund对偶性为理解活性布朗粒子在边界处的积累现象提供了有力框架，建立了吸收边界和硬壁边界问题之间的直接联系，揭示了活性运动对首次通过时间和稳态分布的影响机制。
...
### [57] [Direct Boltzmann inversion method from particle configurations at arbitrary state points](https://arxiv.org/abs/2603.12081)
*Olivier Coquand,Davide Paolino,Ludovic Berthier*

Main category: cond-mat.stat-mech

TL;DR: 提出一种直接玻尔兹曼反演方法，通过粒子配置数据推断相互作用势，无需迭代模拟，基于对关联函数的两种独立估计的一致性。


<details>
  <summary>Details</summary>
Motivation: 传统迭代玻尔兹曼反演方法需要在每次迭代中执行新的蒙特卡洛模拟，计算成本高。现有方法在高密度条件下可能失效。需要一种计算效率更高、适用范围更广的方法来从粒子配置数据中推断相互作用势。

Method: 提出直接玻尔兹曼反演方法，利用粒子间距离和成对力两种独立途径估计对关联函数，并通过强制这两种估计的一致性来推断相互作用势。该方法不依赖迭代模拟，仅需要粒子间力的表达式。

Result: 方法计算成本低且易于实现，适用于任意状态点（包括高密度条件）。通过计算机模拟对多种测试势进行了基准测试验证了方法的性能。讨论了实际应用和具体实现细节。

Conclusion: 该方法具有简单性和普适性，可广泛应用于从粗粒化相互作用势构建到非平衡系统中有效相互作用的推断等多个领域。
...
### [58] [Non-Markovian Entropy Dynamics in Living Systems from the Keldysh Formalism](https://arxiv.org/abs/2603.12184)
*Feiyi Liu,Min Guo,Hongwei Tan,Yang Wang*

Main category: cond-mat.stat-mech

TL;DR: 该研究基于Keldysh泛函形式和随机热力学，为非平衡生物系统的熵动力学建立了一个非马尔可夫理论框架，揭示了活性生物涨落的熵产生特征。


<details>
  <summary>Details</summary>
Motivation: 生命系统是开放的非平衡系统，与环境持续交换能量、物质和信息，导致具有记忆和活性涨落的随机动力学。现有理论需要更全面地处理这些复杂特征。

Method: 采用Keldysh泛函形式和随机热力学，构建包含有色环境噪声、记忆依赖耗散和多体相互作用的非马尔可夫理论框架，得到广义朗之万动力学和非马尔可夫主方程。

Result: 推导出熵产生率的精确频域表达式，证明涨落-耗散关系的违反是活性生物涨落的直接热力学特征；环境记忆增强低频涨落和熵产生，导致动力学不稳定附近的临界慢化。

Conclusion: 该框架为生命系统的熵"浴缸"图像提供了微观物理基础，并将熵演化与非平衡动力学中的发育、衰老和死亡过程联系起来。
...
### [59] [Emergent criticality in the Aubry-André model with periodic modulation](https://arxiv.org/abs/2603.12085)
*Sitaram Maity,Nilanjan Roy,Tapan Mishra*

Main category: cond-mat.dis-nn

TL;DR: 在强周期调制下，准周期系统中的Aubry-André模型出现临界相，具有多重分形本征态和奇异连续谱，属于临界Harper模型的普适类


<details>
  <summary>Details</summary>
Motivation: 研究准周期系统中临界性的工程化机制，探索在额外周期调制下如何保持或产生新的临界行为

Method: 分析Aubry-André模型在强周期调制极限下的行为，研究周期为N的额外势场对谱结构的影响

Result: 发现强调制临界相具有多重分形本征态和奇异连续谱；额外周期势将谱折叠为N个带，每个带内出现N个Hofstadter蝴蝶

Conclusion: 揭示了在准周期系统中工程化稳健临界性和谱复制的通用机制
...
### [60] [Thermalisation as Diffusion in Hilbert Space](https://arxiv.org/abs/2603.12234)
*Aleksey Lunkin*

Main category: cond-mat.dis-nn

TL;DR: 提出了一种超越标准马尔可夫和费米黄金规则的微观热化理论，用于描述温度计与多体浴的耦合，通过随机矩阵元素建模推导出扩散传播子表达式，预测热化时间尺度由典型能级展宽倒数决定。


<details>
  <summary>Details</summary>
Motivation: 现有热化理论通常基于马尔可夫近似和费米黄金规则，这些假设在多体系统中可能不成立。需要发展超越这些限制的微观理论来更准确地描述温度计与多体浴耦合时的热化过程。
...
Result: 理论预测热化时间尺度由典型能级展宽的倒数设定，并给出了全局平衡的非马尔可夫推广。精确对角化测试显示理论与预测结果吻合良好，验证了理论的有效性。

Conclusion: 该研究建立了超越标准近似的热化微观理论框架，揭示了相互作用诱导的能级展宽分布在热化过程中的核心作用，为理解多体系统中的非马尔可夫热化动力学提供了新视角。
...
### [61] [UV/IR relations from the worldsheet](https://arxiv.org/abs/2603.11157)
*Christian Aoufia,Ivano Basile,Giorgio Leone,Matteo Lotito*

Main category: hep-th

TL;DR: 该论文推导了弦理论低能有效作用的普适标度关系，将真空能和规范耦合与高阶导数威尔逊系数联系起来，这些关系在弦微扰论单圈水平上成立，并支持涌现弦猜想和沼泽地原理。


<details>
  <summary>Details</summary>
Motivation: 研究弦理论低能有效作用中的普适关系，旨在连接真空能、规范耦合和高阶导数威尔逊系数，为涌现弦猜想提供更强证据，并将UV/IR混合与沼泽地原理联系起来。

Method: 利用弦微扰论的单圈计算，基于世界面的模不变性和共形不变性推导标度关系。这些关系独立于具体的低能相，在物种极限下变得非平凡。方法还扩展到更高圈阶，并考虑可能的开弦贡献。

Result: 得到了连接真空能和规范耦合与高阶导数威尔逊系数的普适标度关系。这些关系支持涌现弦猜想，将UV/IR混合与沼泽地原理联系起来，并导致参数不等式，这些不等式重现了全息界限，支持磁弱引力猜想和暗维度场景。

Conclusion: 弦理论低能有效作用中存在普适的标度关系，这些关系源于世界面的模不变性和共形不变性，为涌现弦猜想提供了实质性证据，并建立了UV/IR混合与沼泽地原理的深层联系，为研究强耦合提供了通过对偶性的途径。
...
### [62] [Alice in Warpland: KK modes, Warped Compactifications and the Swampland](https://arxiv.org/abs/2603.11163)
*Salvatore Raucci,Ignacio Ruiz,Irene Valenzuela*

Main category: hep-th

TL;DR: 研究弯曲紧致化中Kaluza-Klein塔的渐近行为，发现强弯曲可能违反锐化距离猜想边界，但当高维势能满足禁止渐近加速膨胀条件时，该边界仍被满足。


<details>
  <summary>Details</summary>
Motivation: 研究弯曲紧致化中Kaluza-Klein塔的渐近行为，特别是整体解紧致化极限下的质量标度，探讨弯曲如何影响锐化距离猜想的边界条件。

Method: 聚焦于整体解紧致化极限，推导大KK动量下KK质量在低维普朗克单位中的标度。对于由高维指数势能源的余维一弯曲背景，显式求解内部轮廓，获得塔的指数质量衰减率λ_KK关于模空间距离的闭式表达式。

Result: 弯曲会降低λ_KK相对于非弯曲情况，足够强的弯曲原则上可能违反锐化距离猜想边界。但当高维势能满足禁止渐近加速膨胀条件时，锐化边界仍被精确满足。对于更高余维弯曲背景，渐近KK标度保持不变。

Conclusion: 建立了锐化距离猜想与一高维中强de Sitter条件之间的直接联系，表明弯曲紧致化中KK塔的渐近行为与高维势能的渐近性质密切相关。
...
### [63] [Holes in Calabi-Yau Effective Cones](https://arxiv.org/abs/2603.11173)
*Naomi Gendler,Elijah Sheridan,Michael Stillman,David H. Wu*

Main category: hep-th

TL;DR: 研究Calabi-Yau三维流形有效锥中的"洞"（非全纯除子类），这些除子类在有效锥中但自身无效（无全局截面），在弦论非微扰势中具有重要性。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于弦论中非微扰势的作用，特别是Calabi-Yau流形中除子在有效锥中的行为。作者关注那些在有效锥中但自身无效的除子类，称为"洞"。

Method: 使用环面超曲面Calabi-Yau三维流形集合进行研究，证明洞存在的必要和充分条件，展示从极小模型纲领得出的结果，证明一类洞形成半群，并提供依赖于模的洞四维环体积界限。

Result: 给出了几何实例，其中有效锥中的某些除子类自身无效；表征了洞在环面超曲面Calabi-Yau三维流形集合中的行为；证明了洞存在的必要和充分条件；展示了洞的后果；证明了一类洞形成半群（推测包含所有洞）；提供了洞四维环体积的模依赖界限。

Conclusion: Calabi-Yau三维流形有效锥中存在"洞"（非全纯除子类），这些洞具有特定的数学结构和性质，在弦论非微扰势中可能具有重要物理意义，需要进一步研究其完整分类和物理含义。
...
### [64] [The role of p_1-structures in 3-dimensional Chern-Simons theories](https://arxiv.org/abs/2603.11291)
*Daniel S. Freed,Constantin Teleman*

Main category: hep-th

TL;DR: 该论文基于cobordism假设构建完全局域Chern-Simons理论，并探讨其物理动机：玻色情况下的Yang-Mills加Chern-Simons项和费米情况下的自由Majorana-Weyl旋量场。


<details>
  <summary>Details</summary>
Motivation: 构建完全局域的Chern-Simons理论，为拓扑场论提供物理基础。具体动机包括：1) 玻色情况：Yang-Mills理论加Chern-Simons项；2) 费米情况：自由Majorana-Weyl旋量场；3) 利用Witten的"引力Chern-Simons理论"从物理理论获得拓扑场论。

Method: 使用cobordism假设（cobordism hypothesis）作为核心数学工具构建理论框架。引入切向结构（tangential structures）和可逆场论（invertible field theories）的概念，特别是Witten提出的"引力Chern-Simons理论"。

Result: 成功构建了完全局域的Chern-Simons理论，为拓扑场论提供了严格的数学基础。展示了如何从物理理论（如Yang-Mills加Chern-Simons和Majorana-Weyl旋量场）获得拓扑场论。

Conclusion: cobordism假设为构建完全局域Chern-Simons理论提供了强大框架，连接了物理理论与拓扑场论。该工作为理解Chern-Simons理论的数学结构和物理实现提供了新视角。
...
### [65] [The phase diagram of the D1-D5 CFT and localized black holes](https://arxiv.org/abs/2603.11181)
*Ofer Aharony,Ronny Frumkin,Jonathan Mehl*

Main category: hep-th

TL;DR: 分析D1-D5 CFT中微正则系综在不同能量下的相结构，研究黑洞解及其相变，特别关注环面尺寸与AdS尺度之比的影响


<details>
  <summary>Details</summary>
Motivation: 研究AdS₃×S³×T⁴背景下D1-D5 CFT的相结构，理解黑洞在不同能量尺度下的行为，特别是当黑洞增长时如何填充S³或T⁴空间维度

Method: 分析微正则系综中的相结构，研究黑洞解（包括均匀和非均匀黑洞），结合已知解并讨论未知解的性质，特别关注环面尺寸与AdS半径之比的影响

Result: 发现当环面远大于AdS半径时，存在一个能量范围，其中典型态是一种新颖相，由具有S⁵×S³视界拓扑的黑洞晶格描述，熵与能量呈线性关系，系数为AdS半径量级

Conclusion: D1-D5 CFT的相结构既有与高维AdS空间相似的特征，也有独特之处，特别发现了在特定参数范围内存在新颖的黑洞晶格相
...
### [66] [Large-$N$ Torus Knots in Lens Spaces and Their Quiver Structure](https://arxiv.org/abs/2603.11997)
*Ritabrata Bhattacharya,Suvankar Dutta,Naman Pasari,Nitin Verma*

Main category: hep-th

TL;DR: 本文研究透镜空间S³/ℤₐ中环面结不变量，在Chern-Simons理论框架下推导了(α,β)环面结不变量的一般表达式，发现大N极限下不变量可简化为S³中(α,α+pβ)环面结不变量，且生成函数具有类似quiver配分函数的结构。


<details>
  <summary>Details</summary>
Motivation: 研究透镜空间背景下的环面结不变量，探索Chern-Simons理论中结不变量的结构特性，特别是大N极限下的简化规律以及与quiver结构的联系。

Method: 利用手术和透镜空间的模描述方法，在Chern-Simons理论框架下推导(α,β)环面结在S³/ℤₐ中的不变量表达式，分析大N极限下的简化形式，通过变量重定义研究生成函数的quiver结构。

Result: 推导出透镜空间中环面结不变量的一般表达式；发现大N极限下不变量可简化为S³中(α,α+pβ)环面结不变量；证明生成函数具有quiver配分函数结构，且关联的quiver与Chern-Simons理论的秩N和水平k无关。

Conclusion: 透镜空间中的环面结不变量在大N极限下呈现普适简化形式，其生成函数具有quiver结构，这为识别和确定透镜空间中环面结的底层quiver结构提供了直接途径。
...
### [67] [One-loop mass corrections and decay widths of Type II heavy string states](https://arxiv.org/abs/2603.11343)
*Massimo Bianchi,Maurizio Firrotta,Lorenzo Grimaldi*

Main category: hep-th

TL;DR: 系统研究弦论中高阶自旋态的单圈质量修正，重点关注Type-II弦理论NS-NS扇区第一Regge轨迹态，计算至N=10能级并分析其行为。


<details>
  <summary>Details</summary>
Motivation: 研究弦论中高阶自旋态的单圈质量修正问题，这些修正的虚部有限（与树图衰变宽度相关），但实部存在红外发散，需要正则化和重整化处理。

Method: 主要关注Type-II弦理论NS-NS扇区第一Regge轨迹态；利用椭圆函数和格点求和性质推导插入点积分的闭形式表达式；应用弦论中的$i\varepsilon$正则化方案处理环面模参数的红外发散积分。

Result: 计算至N=10能级的质量修正，分析其随N增加的行为；推测低自旋态间存在混合，并提出单圈质量矩阵可能由某种随机矩阵理论支配的猜想。

Conclusion: 系统建立了弦论高阶自旋态单圈质量修正的计算框架，揭示了红外发散的正则化方法，并提出了质量矩阵可能遵循随机矩阵理论的有趣猜想。
...
### [68] [Integrability from Homotopy Algebras](https://arxiv.org/abs/2603.12113)
*Luigi Alfonsi,Leron Borsten,Mehran Jalali Farahani,Hyungrok Kim,Martin Wolf,Charles Alastair Stephen Young*

Main category: hep-th

TL;DR: 该论文建立了半全纯陈-西蒙斯理论与主手征模型之间的显式拟同构，通过循环L∞代数框架为二维可积系统提供了同伦代数视角的具体实例。


<details>
  <summary>Details</summary>
Motivation: 研究同伦代数方法在场论中的应用，特别是探索半全纯陈-西蒙斯理论与主手征模型之间的深层数学联系，为理解二维可积系统提供新的代数框架。

Method: 通过建立两个理论之间的显式拟同构，具体构造了连接半全纯陈-西蒙斯理论和主手征模型的循环L∞代数对应关系，并直接推导出Lax联络。

Result: 成功建立了两个理论循环L∞代数之间的显式拟同构，该同构直接给出了Lax联络，为通过同伦代数透镜研究二维系统可积性提供了具体实例。

Conclusion: 该工作展示了同伦代数方法在研究场论可积性方面的有效性，为理解半全纯陈-西蒙斯理论与主手征模型的深层联系提供了严格的数学框架，并开辟了通过代数结构研究可积系统的新途径。
...
### [69] [Diving into booklet wormholes](https://arxiv.org/abs/2603.11459)
*Libo Jiang,Yan Liu*

Main category: hep-th

TL;DR: 该论文扩展了booklet虫洞作为GHZ态全息对偶的研究，重点分析了物质场的连接条件，发现GHZ态的对称性要求体空间具有特殊Killing矢量场，这些对称性导致多向界面处出现前所未有的量子非局域连接条件，并讨论了通过边界变形使booklet虫洞可穿越的方法。


<details>
  <summary>Details</summary>
Motivation: 基于ArXiv:2508.17898提出的booklet虫洞作为GHZ态的全息对偶，本文旨在深入研究该几何结构的特性，特别是物质场的连接条件，以更好地理解量子引力中多体纠缠态的全息实现。

Method: 通过分析GHZ态的对称性要求，推导体空间必须满足的特殊Killing矢量场条件；研究多向界面处的量子非局域连接条件；探讨通过边界变形实现虫洞可穿越性的方法。

Result: 发现GHZ态的对称性要求体空间具有标准流形无法实现的特殊Killing矢量场；这些对称性导致多向界面处出现前所未有的量子非局域连接条件：从不同视界进入的观测者会感知虫洞内部的不同状态；通过边界变形可以使booklet虫洞可穿越，从一页注入的局域波包通常演化为其余各页上的非局域混合态，信息编码在不同页之间的纠缠中。

Conclusion: booklet虫洞作为GHZ态的全息对偶展现出独特的几何和量子特性，其连接条件反映了量子引力中多体纠缠的非局域本质，为理解全息对偶中多向纠缠态的几何实现提供了新视角。
...
### [70] [Gauge invariant non-perturbative Wilson action in quantum electrodynamics](https://arxiv.org/abs/2603.11626)
*Sorato Nagao,Hiroshi Suzuki*

Main category: hep-th

TL;DR: 使用GFERG方法研究QED中规范不变的1PI Wilson作用的RG流，在large N_f近似下求解GFERG方程，得到规范不变临界指数和红外固定点处的规范不变1PI Wilson作用


<details>
  <summary>Details</summary>
Motivation: 研究量子电动力学中规范不变的Wilson作用在重整化群流下的行为，确保规范不变性在RG流中精确保持，这对于理解规范理论的非微扰特性至关重要

Method: 采用梯度流精确重整化群(GFERG)方法，研究规范或BRST不变的1PI Wilson作用的非微扰近似，在large N_f近似下求解GFERG方程，包括领头阶和部分次领头阶

Result: 在D<4时空维度下，获得了规范不变的临界指数和红外固定点处的规范不变1PI Wilson作用，规范不变性在RG流中精确保持

Conclusion: GFERG方法能够有效研究规范理论的非微扰RG流，在large N_f近似下成功获得了规范不变的临界指数和红外固定点行为，为理解规范理论的非微扰特性提供了新视角
...
### [71] [Quantum backreaction and stability of topological wormholes](https://arxiv.org/abs/2603.11724)
*Haris Mehulic,Tomislav Prokopec*

Main category: hep-th

TL;DR: 研究量子效应对时间型拓扑虫洞稳定性的影响，计算标量场真空涨落的一圈量子反作用，分析量子修正对虫洞稳定性和可穿越性的影响。


<details>
  <summary>Details</summary>
Motivation: 研究经典各向异性流体支持的虫洞在量子修正下的稳定性问题，探讨量子效应如何影响虫洞的动力学行为和可穿越性。

Method: 使用维度正规化方法计算最小耦合大质量标量场真空涨落的一圈能动张量，进行重整化并识别必要的引力抵消项，然后求解半经典爱因斯坦方程到ħ的一阶。

Result: 量子效应可诱导负或正的角向压力，分别倾向于使虫洞失稳或稳定；经典可穿越虫洞在考虑量子反作用后仍保持可穿越性。

Conclusion: 量子修正对虫洞稳定性有显著影响，具体效应取决于有限抵消项的选择；需要进一步研究半经典引力方程自洽解的情况。
...
### [72] [On the Sugawara Current Algebra Proposal for M-Theory](https://arxiv.org/abs/2603.11923)
*Keith Glennon*

Main category: hep-th

TL;DR: 该论文探讨了M理论是否可能基于$E_{11} \otimes_s l_1$代数实现Sugawara型流代数表述，并研究了在包含广义坐标的系统性设置中推导此类流代数关系的可能性。


<details>
  <summary>Details</summary>
Motivation: 受E理论中广义坐标作用的启发，研究者希望探索在系统性包含广义坐标的框架下，能否推导出基于$E_{11} \otimes_s l_1$的Sugawara型流代数关系，从而为M理论提供新的代数表述基础。

Method: 采用刚性$E_{11}$模型，其中广义坐标被视为在刚性对称性下惰性（与E理论不同），并仔细分析了Schwinger项中双线性形式的性质，特别是$E_{11}$ Cartan-Killing形式到$E_{11} \otimes_s l_1$的任何自然伴随不变扩展的退化性问题。

Result: 成功在刚性$E_{11}$模型中实现了此类构造，证明了在广义坐标惰性处理的框架下可以推导出Sugawara型流代数关系。同时发现Schwinger项中的双线性形式需要更仔细的审查，因为任何从$E_{11}$ Cartan-Killing形式到$E_{11} \otimes_s l_1$的自然伴随不变扩展都是退化的。

Conclusion: M理论可能确实允许基于$E_{11} \otimes_s l_1$的Sugawara型流代数表述，但需要仔细处理双线性形式的退化性问题。刚性$E_{11}$模型为这一构造提供了可行框架，但与E理论中广义坐标的变换性质存在重要差异。
...
### [73] [Soft cutoffs in the covariant phase space of dynamical reference frames](https://arxiv.org/abs/2603.11966)
*Kang Liu,Wei Guo,Xiao-Mei Kuang*

Main category: hep-th

TL;DR: 该论文构建了包含波动边界和软截断的协变理论，通过引入动力学参考框架(DRFs)将协变作用量从硬截断推广到软截断形式，恢复了微分同胚协变性，并在协变相空间形式中推导了软截断理论的守恒荷。


<details>
  <summary>Details</summary>
Motivation: 传统协变理论通常使用硬截断边界，这限制了处理波动边界和软截断情况的能力。作者旨在构建一个更一般的框架，能够包含边界波动和软截断，同时保持微分同胚协变性，从而一致地定义子系统。

Method: 引入动力学参考框架(DRFs)，使用涂抹函数及其对应的算符展开将协变作用量从硬截断推广到软截断形式。通过限制DRFs及其相关线性MCFs的形式，并对涂抹函数施加适当的边界条件来恢复协变性。在协变相空间形式中推导守恒荷，并引入额外的点依赖关系来解决边界拉格朗日量引起的模糊性。

Result: 成功构建了包含波动边界和软截断的协变理论框架，恢复了关系时空中的微分同胚协变性。推导了软截断理论的守恒荷，并通过引入点依赖关系解决了边界拉格朗日量引起的模糊性，确保了即使在波动边界条件下守恒荷的可积性。在广义相对论背景下，建立了渐近边界处全息重整化结果与软截断程序导出的Noether荷一致的条件。

Conclusion: 该研究提供了一个处理波动边界和软截断的协变理论框架，通过动力学参考框架恢复了微分同胚协变性，解决了边界拉格朗日量引起的守恒荷模糊性问题，并在广义相对论中建立了与全息重整化的联系，为一致定义子系统提供了理论基础。
...
### [74] [More on Bulk Local State in Flat Holography](https://arxiv.org/abs/2603.11993)
*Peng-Xiang Hao,Kotaro Shinmyo,Yu-ki Suzuki,Shunta Takahashi*

Main category: hep-th

TL;DR: 该论文重新审视并扩展了平直全息中体局部态的构造，通过引入对偶基解决三维情况下的标度失配问题，并在任意维度构建了体局部态，建立了诱导表示作为平直全息体重建的正确代数基础。


<details>
  <summary>Details</summary>
Motivation: 研究平直全息中体局部态的构造，解决AdS最高权条件平直极限下的标度失配问题，建立适用于任意维度的统一框架。

Method: 采用诱导表示方法，从AdS最高权条件的平直极限出发。在三维中引入对偶基解决标度失配；在高维中分别在动量基和替代的tilde基中显式构造体局部态，通过Riemann和将离散后裔展开转换为连续动量表示。

Result: 成功解决了三维情况下的标度失配问题，获得了正确的格林函数；在高维中显式构造了体局部态，证明了平直极限在AdS_{d+1}构造中在后裔层级上非均匀，通过标度窗口n∼l的Riemann和处理恢复了质量传播子；tilde基可无缝推广到任意维度。

Conclusion: 诱导表示是平直全息体重建的正确代数基础，所建立的统一框架适用于任意维度，为平直全息提供了坚实的理论基础。
...
### [75] [The disk 1-point function in timelike Liouville theory](https://arxiv.org/abs/2603.12084)
*Gaston Giribet,Bruno Sivilotti*

Main category: hep-th

TL;DR: 计算了类时Liouville理论中的圆盘1点函数，通过库仑气体形式并解析延拓筛选算符数量，得到了显式公式


<details>
  <summary>Details</summary>
Motivation: 研究类时Liouville理论中的圆盘1点函数，验证其反射对称性、自对偶性质、靴带移位方程，并与已知结果建立联系

Method: 采用库仑气体形式体系，通过对筛选算符数量进行解析延拓，推导出显式公式

Result: 得到了满足正确反射对称性、具有预期自对偶性质、满足靴带移位方程的显式公式，在适当极限下还原为已知结果，零宇宙学常数极限下重现arXiv:2505.09390的结果

Conclusion: 成功计算了类时Liouville理论的圆盘1点函数，验证了理论的自洽性，建立了与现有结果的联系
...
### [76] [Hawking Radiation from Tunneling in Black Hole Quantum Mechanics](https://arxiv.org/abs/2603.12199)
*Chong-Sun Chu*

Main category: hep-th

TL;DR: 该论文提出用量子力学中的模糊球和半满费米海描述量子黑洞，通过模糊球系统隧穿到较小系统的过程解释霍金辐射导致的量子黑洞衰变，并利用磁单极子提供零模来保持费米子数守恒，从而得到与Page结果一致的隧穿率。


<details>
  <summary>Details</summary>
Motivation: 基于Chu:2024qil提出的用量子模糊球和半满费米海描述量子黑洞的框架，需要解释黑洞如何通过霍金辐射进行量子衰变。关键挑战在于费米海收缩时费米子数守恒会导致隧穿振幅为零，需要找到能吸收多余费米态的机制。

Method: 提出模糊球系统隧穿到较小系统的量子隧穿过程描述黑洞衰变。引入模糊球上的磁单极子提供精确数量的零模来吸收多余的费米态，从而固定隧穿路径。使用实时隧穿过程公式化方法，超越概率描述，确定多部分霍金量子的完整波函数。

Result: 磁单极子机制成功解决了费米子数守恒导致的隧穿振幅为零问题。计算得到的隧穿率精确再现了Page关于黑洞半经典衰变率的结果。霍金辐射在概率层面表现为霍金温度下的玻尔兹曼分布。通过实时公式化，可以显式展示量子力学中的幺正性。

Conclusion: 模糊球+半满费米海模型为量子黑洞提供了自洽的量子力学描述，其中磁单极子介导的隧穿过程成功解释了霍金辐射导致的量子衰变，隧穿率与半经典结果一致，且幺正性在量子力学框架中显式保持。
...
### [77] [Progress of ambient-pressure superconductivity in bilayer nickelate thin films](https://arxiv.org/abs/2603.11235)
*Wenyuan Qiu,Dao-Xin Yao*

Main category: cond-mat.supr-con

TL;DR: 本文综述了双层镍酸盐La₃Ni₂O₇薄膜在常压超导方面的最新进展，重点介绍了外延应变工程如何实现常压超导、实验表征方法、Tc提升策略以及理论研究。


<details>
  <summary>Details</summary>
Motivation: 继块体La₃Ni₂O₇在高压下发现超导性后，研究其薄膜材料在常压下的超导特性具有重要意义。通过外延应变工程实现常压超导，为探索高Tc超导机制提供了新的可调控平台。

Method: 采用外延应变工程方法，利用SrLaAlO₄等衬底产生的压应变稳定超导态；通过ARPES等实验技术表征费米面拓扑结构；结合理论计算研究电子结构和配对对称性。

Result: 成功在La₃Ni₂O₇薄膜中实现常压超导，压缩应变能稳定超导态；实验表征揭示了费米面拓扑的争议性结果；通过优化制备条件提升了Tc；理论研究为理解超导机制提供了重要见解。

Conclusion: 双层镍酸盐薄膜通过外延应变工程实现了常压超导，成为高度可调控的高Tc超导研究平台，为深入理解镍基超导机制和开发新型超导材料开辟了新途径。
...
### [78] [Pseudo Point Nodal Superconducting Gap in Spin-Triplet UTe$_2$](https://arxiv.org/abs/2603.11278)
*S. Hosoi,K. Imamura,M. M. Bordelon,E. D. Bauer,S. M. Thomas,F. Ronning,P. F. S. Rosa,R. Movshovich,I. Vekhter,Y. Matsuda*

Main category: cond-mat.supr-con

TL;DR: UTe₂超导体热导率测量揭示其具有完全能隙结构，但存在伪点节点特征，最小能隙约为特征能隙的0.1倍


<details>
  <summary>Details</summary>
Motivation: 解决关于UTe₂超导体能隙结构（特别是点节点存在与否）的矛盾报道，以理解其序参量对称性和拓扑性质

Method: 使用高质量UTe₂单晶进行高分辨率热导率测量，温度低至约50 mK，通过体相定向输运研究能隙各向异性

Result: b轴热导率κ_b/T在T→0时残余电导率可忽略，温度依赖性与沿b轴的小超导能隙一致；低场下H∥a时出现阈值场，表现为拐点特征；阈值场以下κ_b/T对所有场方向保持各向同性，阈值场以上则出现沿场方向和垂直场方向的强各向异性

Conclusion: UTe₂表现出具有伪点节点结构的完全能隙态，能隙最小值接近但不为零，最小能隙Δ_min/Δ_0∼0.1沿b轴，这一不寻常的能隙结构为理解该自旋三重态超导体的配对机制和拓扑性质提供了关键见解，并排除了非幺正混合配对对称性
...
### [79] [Quantum Oscillations and Superconductivity in YPtBi Under Pressure](https://arxiv.org/abs/2603.11464)
...
<details>
  <summary>Details</summary>
Motivation: 研究压力对拓扑半金属YPtBi的影响，探索其拓扑性质的调控机制。YPtBi因其新颖的超导和正常态性质而备受关注，其强自旋轨道耦合导致的能带反转使得费米面附近存在j=3/2准粒子，可能形成角动量高于单重态或三重态的Cooper对。

Method: 对高质量YPtBi样品进行高达2.08 GPa的高压磁输运测量和Shubnikov-de Haas效应测量，分析压力对电阻率、量子振荡幅度、上临界场和Dingle温度的影响。

Result: 随着压力增加，观察到低温下电阻率趋向更绝缘的行为，同时量子振荡幅度被抑制。上临界场降低，Dingle温度显著增加。这些变化表明能带反转减弱，YPtBi的拓扑性质可能被压力调控。

Conclusion: 压力是理解YPtBi及其他相关half-Heusler化合物拓扑性质的有用工具，能带反转的减弱和拓扑性质的调控为研究拓扑材料的性质调控提供了新途径。
...
### [80] [Double-twisted surface spectrum from hybridized Majorana Kramers pairs and wallpaper fermions](https://arxiv.org/abs/2603.11637)
...
<details>
  <summary>Details</summary>
Motivation: 研究壁纸费米子（拓扑非对称晶体绝缘体的表面准粒子）在超导状态下的表面态特性，特别关注其与已知超导拓扑（晶体）绝缘体（如Cu_xBi_2Se_3和Sn_{1-x}In_xTe）的差异。

Method: 基于空间群P4/mbm（No.127）的紧束缚模型进行理论分析，利用壁纸群p4g的对称性和一维拓扑不变量，分析四种允许的现场配对势，重点研究A1u表示下的表面态特性。

Result: 发现A1u表示下壁纸费米子与两对Majorana Kramers对共存，它们的杂化产生双扭曲表面态，在表面态密度中形成四个峰；镜像陈数为零，表明系统实现了无镜像螺旋性的表面态。

Conclusion: 超导壁纸费米子表现出独特的无镜像螺旋性表面态，这使其区别于其他超导拓扑（晶体）绝缘体，为拓扑超导表面态研究提供了新范例。
...
### [81] [Theoretical proposal of superconductivity in hole-doped reduced bilayer nickelate La3Ni2O6: a manifestation of orbital-space bilayer model with incipient bands](https://arxiv.org/abs/2603.11771)
*Shu Kamiyama,Reo Kohno,Yuto Hoshi,Kensei Ushio,Daiki Nakaoka,Hirofumi Sakakibara,Kazuhiko Kuroki*

Main category: cond-mat.supr-con

TL;DR: 该研究提出La₃Ni₂O₆可作为轨道空间双层模型(OSBM)的超导候选材料，在适当空穴掺杂下通过轨道间相互作用实现s±波超导。


<details>
  <summary>Details</summary>
Motivation: 探索多轨道Hubbard模型与双层Hubbard模型之间的对应关系，并寻找能够实现轨道空间双层模型描述的新型超导材料。La₃Ni₂O₆因其特殊的晶体结构（缺少外层顶端氧）可能提供实现这种机制的平台。

Method: 基于第一性原理计算构建紧束缚模型，获得Ni d_{x²-y²}轨道与其他d轨道之间的能级偏移ΔE。采用涨落交换近似计算研究超导配对机制，并分析晶体结构在原子替代和压力下的能量与动力学稳定性。

Result: La₃Ni₂O₆中由于缺少外层顶端氧，导致d_{x²-y²}轨道与其他d轨道之间存在大的能级偏移ΔE。在初生带情况下，轨道间相互作用驱动s±波超导，超导能隙函数在d_{x²-y²}轨道带与其他d轨道带之间改变符号。晶体结构在适当条件下保持稳定。

Conclusion: La₃Ni₂O₆是轨道空间双层模型描述的超导候选材料，其超导机制与La₃Ni₂O₇完全不同，通过轨道间相互作用在初生带情况下实现s±波配对，为探索新型超导材料提供了新方向。
...
### [82] [Multiple Magnetic Transitions in the Trilayer Nickelate Pr$_4$Ni$_3$O$_{10}$ Revealed by Muon-Spin Rotation](https://arxiv.org/abs/2603.11823)
...
TL;DR: 通过μSR技术研究了三层Ruddlesden-Popper镍酸盐Pr₄Ni₃O₁₀在常压和最高2.2 GPa静水压下的磁性相变，发现三个磁性转变温度，其中SDW转变表现出弱一级相变特征，压力会线性抑制SDW转变温度和有序磁矩。


<details>
  <summary>Details</summary>
Motivation: 研究Pr₄Ni₃O₁₀镍酸盐材料的磁性相变行为，特别是在静水压力下的响应，以理解其自旋密度波（SDW）不稳定性及其与晶格压缩的关系。

Method: 采用μ子自旋转动/弛豫（μSR）技术，在常压和最高2.2 GPa的静水压力下，对Pr₄Ni₃O₁₀样品进行磁性测量，通过弱横向场测量检测热滞现象。

Result: 常压下观察到三个磁性转变：SDW有序起始温度T_SDW≈158 K，中间温度转变T*≈90-100 K，低温转变T_SDW^Pr≈25-27 K。T_SDW转变宽度窄（0.65(4) K），存在0.27(6) K的热滞，表明弱一级相变行为。压力线性抑制T_SDW（dT_SDW/dp = -4.9(1) K/GPa）和有序磁矩M（dlnM/dp = -2.0(5)×10⁻² GPa⁻¹）。

Conclusion: Pr₄Ni₃O₁₀的自旋密度波不稳定性在压缩下逐渐减弱，T_SDW转变表现出弱一级相变特征，Pr亚晶格有序在低温下增强了层间相干性。
...
### [83] [Phase stiffness in flat-band superconductors with nodal pairing](https://arxiv.org/abs/2603.11986)
*A. A. Zyuzin,A. Yu. Zyuzin*

Main category: cond-mat.supr-con

TL;DR: 研究两带系统中动量依赖杂化对Bogoliubov准粒子谱的影响，发现平带准粒子谱中可能出现抛物线节点，导致超导相位刚度在低温下呈二次温度依赖关系。


<details>
  <summary>Details</summary>
Motivation: 研究具有动量依赖杂化的两带系统中平带超导的特性，特别是平带准粒子谱的独特行为和其对无序的敏感性。

Method: 研究一个包含色散带和平带的系统，其中两带之间存在动量依赖的杂化，同时考虑带内Cooper配对。分析Bogoliubov准粒子谱，特别是平带准粒子谱中抛物线节点的形成机制。

Result: 发现平带准粒子谱中可能出现抛物线节点，这导致超导相位刚度在低温下呈现二次温度依赖关系。同时发现非磁性无序会诱导Machida-Shibata深亚能隙共振，表明平带超导对无序具有敏感性。

Conclusion: 动量依赖杂化与带内配对的相互作用可以产生独特的平带准粒子谱特征，包括抛物线节点和二次温度依赖的相位刚度。平带超导对无序敏感，非磁性无序会诱导深亚能隙共振。
...
### [84] [Quantum interference in a twisted high-Tc SQUID senses emergent interfacial order](https://arxiv.org/abs/2603.12092)
...
TL;DR: 该研究通过在扭曲Bi₂Sr₂CaCu₂O₈₊δ界面构建超导量子干涉器件，首次观测到手性超导序和π相位差，并实现了接近77K的高性能磁通传感器。


<details>
  <summary>Details</summary>
Motivation: 探索扭曲范德华材料界面中的新兴量子现象，特别是研究扭曲铜氧化物超导体中尚未充分探索的界面超导序，利用高质量扭曲界面的制备技术来揭示传统单约瑟夫森结器件无法探测的关键超导特性。

Method: 在扭曲Bi₂Sr₂CaCu₂O₈₊δ（Bi-2212）铜氧化物超导体界面上制造超导量子干涉器件，通过测量开关电流和微分电阻的磁场调制，分析两个约瑟夫森结臂之间的相位关系，探测界面超导序的对称性。

Result: 观测到SQUID两个约瑟夫森结臂之间存在π相位差，表明存在手性超导序；发现库珀对共隧穿效应和时间反演对称性破缺的新兴超导序；器件在接近77K时表现出优异的磁通噪声灵敏度（~1.5 μΦ₀/√Hz），可作为高性能磁通传感器。

Conclusion: 扭曲界面为稳定新型超导序提供了有效平台，量子干涉探测方法为理解非常规超导体的微观起源开辟了新途径；该SQUID架构具有广泛适用性，可用于研究其他系统界面处的电荷输运机制和超导序对称性。
...
### [85] [A superconducting half-dome in bilayer nickelates](https://arxiv.org/abs/2603.12196)
...
TL;DR: 在压缩应变双层镍酸盐薄膜中，通过连续调控氧化学计量比，观察到了超导半圆顶结构：增加氧含量逐渐抑制超导性，减少氧含量则驱动颗粒超导-绝缘体转变。


<details>
  <summary>Details</summary>
Motivation: 理解关联电子系统中超导性如何出现和崩溃是凝聚态物理的核心挑战。双层镍酸盐作为高温超导体家族新成员，其显著的氧化学计量比敏感性为调控电子相提供了有效控制参数。

Method: 在压缩应变双层镍酸盐薄膜中，通过连续调控氧化学计量比，系统研究超导性随氧含量变化的相图。使用不同稀土元素组合和有无碱土金属掺杂的样品进行对比研究。

Result: 观察到超导半圆顶结构：从最佳超导态开始，增加氧含量逐渐抑制超导性向金属相转变；减少氧含量则驱动颗粒超导-绝缘体转变，同时保持超导起始温度不变。该半圆顶结构在不同稀土组合和有无碱土金属掺杂的样品中均一致出现。

Conclusion: 双层镍酸盐相图中普遍存在超导半圆顶结构，这源于间隙氧（主导掺杂）与氧空位（主导散射）的对比作用。氧化学计量比作为有效控制参数，为理解关联电子系统中超导性的出现和崩溃提供了新视角。
